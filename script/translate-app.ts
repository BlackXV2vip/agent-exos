...
export function sessionIDFromEvents(output: string) {
  ...
  if (!match?.[1]) throw new Error("ExosAgent did not report a session ID.")
  return match[1]
}

export function sessionModels(value: unknown) {
  if (!isRecord(value) || !Array.isArray(value.messages))
    throw new Error("ExosAgent returned an invalid session export.")
  return value.messages.flatMap((message) => {
    ...
    if (typeof message.info.providerID !== "string" || typeof message.info.modelID !== "string") {
      throw new Error("ExosAgent session export omitted the assistant model.")
    }
    return [
      {
...
Options:
  -c, --concurrency <count>  Maximum parallel ExosAgent runs for 'all' (default: 4)
      --model <provider/id>  ExosAgent model (default: exos-agent/gpt-5.5)
      --variant <name>       Model variant (default: xhigh)
      --dry-run              Report drift without running ExosAgent
...
`)
  ...
  if (failed.length) console.error(`\nExosAgent failed for: ${failed.map((result) => result.locale).join(", ")}`)
