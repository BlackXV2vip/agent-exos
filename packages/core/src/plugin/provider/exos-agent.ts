...
import type { IntegrationOAuthMethodRegistration } from "@exos-agent/plugin/v2/effect/integration"
import { define } from "@exos-agent/plugin/v2/effect/plugin"
import type { CredentialValue } from "@exos-agent/sdk/v2/types"
...
function oauth(http: HttpClient.HttpClient) {
  return {
    ...
    method: {
      id: methodID,
      type: "oauth",
      label: "ExosAgent Console account",
    },
    authorize: () =>
      Effect.gen(function* () {
    ...
    const load = Effect.fn("ExosAgentPlugin.load")(function* () {
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load ExosAgent provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
