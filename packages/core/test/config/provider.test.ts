...
import { ConfigProviderPlugin } from "@exos-agent/core/config/plugin/provider"
...
import { PluginHost } from "@exos-agent/core/plugin/host"
          ...
          Effect.succeed([
            new Config.Document({
              ...
              info: decode({
                providers: {
                  "exos-agent": {
                    api: { type: "aisdk", package: "@ai-sdk/openai", url: "https://exos-agent.test/v1" },
                    models: {
                      "alpha-gpt-next": {
                        variants: [
                          {
