...
You can add external tools to Exos Agent using the _Model Context Protocol_, or MCP. Exos Agent supports both local and remote servers.
...
You can define MCP servers in your [Exos Agent Config](https://exos-agent.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/exos-agent` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
Exos Agent automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Exos Agent will:
...
If the server requires authentication, Exos Agent will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `exos-agent mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, Exos Agent will store the tokens securely in `~/.local/share/exos-agent/mcp-auth.json`.
...
Your MCPs are available as tools in Exos Agent, alongside built-in tools. So you can manage them through the Exos Agent config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Exos Agent to your Sentry account.
