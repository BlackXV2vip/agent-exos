...
Les compétences d'agent permettent à Exos Agent de découvrir des instructions réutilisables à partir de votre dépôt ou de votre répertoire personnel.
...
Exos Agent recherche ces emplacements :

- Configuration du projet : `.exos-agent/skills/<name>/SKILL.md`
- Configuration globale : `~/.config/exos-agent/skills/<name>/SKILL.md`
...
Pour les chemins locaux du projet, Exos Agent remonte de votre répertoire de travail actuel jusqu'à ce qu'il atteigne l'arbre de travail git.
Il charge tout `skills/*/SKILL.md` correspondant dans `.exos-agent/` et tout `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` correspondant en cours de route.

Les définitions globales sont également chargées à partir de `~/.config/exos-agent/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` et `~/.agents/skills/*/SKILL.md`.
...
Créez `.exos-agent/skills/git-release/SKILL.md` comme ceci :
...
Exos Agent répertorie les compétences disponibles dans la description de l'outil `skill`.
...
Contrôlez les compétences auxquelles les agents peuvent accéder à l'aide d'autorisations basées sur des modèles dans `exos-agent.json` :
...
**Pour les agents intégrés** (dans `exos-agent.json`) :
