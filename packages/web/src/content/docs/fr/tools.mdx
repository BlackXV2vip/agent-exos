...
Les outils permettent au LLM d'effectuer des actions dans votre base de code. Exos Agent est livré avec un ensemble d'outils intégrés, mais vous pouvez l'étendre avec [outils personnalisés](/docs/custom-tools) ou [serveurs MCP](/docs/mcp-servers).
...
Voici tous les outils intégrés disponibles dans Exos Agent.
...
:::note
Cet outil n'est disponible que lorsque `EXOS_AGENT_EXPERIMENTAL_LSP_TOOL=true` (ou `EXOS_AGENT_EXPERIMENTAL=true`).
:::
...
:::note
Cet outil n'est disponible que lors de l'utilisation du fournisseur Exos Agent ou lorsque la variable d'environnement `EXOS_AGENT_ENABLE_EXA` est définie sur une valeur véridique (par exemple, `true` ou `1`).

À activer lors du lancement de Exos Agent :
