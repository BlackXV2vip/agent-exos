...
Pour déboguer les problèmes avec Exos Agent, commencez par vérifier les journaux et les données locales qu'il stocke sur le disque.
...
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\exos-agent\log`
...
Vous pouvez définir le niveau de journalisation avec l'option de ligne de commande `--log-level` pour obtenir des informations de débogage plus détaillées. Par exemple, `exos-agent --log-level DEBUG`.

---

## Stockage

exos-agent stocke les données de session et autres données d'application sur le disque à l'emplacement :

- **macOS/Linux** : `~/.local/share/exos-agent/`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\exos-agent`
...
Exos Agent Desktop exécute un serveur Exos Agent local (le side-car `exos-agent-cli`) en arrière-plan. La plupart des problèmes sont causés par un plugin qui se comporte mal, un cache corrompu ou un mauvais paramètre du serveur.
...
- macOS uniquement : menu `ExosAgent` -> **Recharger la vue Web** (aide si l'interface utilisateur est vide/gelée).
...
- **macOS/Linux** (anciennes installations) : `~/.local/share/exos-agent/exos-agent.jsonc`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\exos-agent\exos-agent.jsonc`
...
Exos Agent peut également charger des plugins locaux à partir du disque. Écartez-les temporairement (ou renommez le dossier) et redémarrez l'application de bureau :

- **Plugins mondiaux**
  - **macOS/Linux** : `~/.config/exos-agent/plugins/`
  - **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\exos-agent\plugins`
...
Si la désactivation des plugins ne résout pas le problème (ou si l'installation d'un plugin est bloquée), videz le cache afin que Exos Agent puisse le reconstruire.

1. Quittez complètement Exos Agent Desktop.
2. Supprimez le répertoire cache :

- **macOS** : Finder -> `Cmd+Shift+G` -> coller `~/.cache/exos-agent`
- **Linux** : supprimez `~/.cache/exos-agent` (ou exécutez `rm -rf ~/.cache/exos-agent`)
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.cache\exos-agent`

3. Redémarrez le bureau Exos Agent.

---

### Résoudre les problèmes de connexion au serveur

Exos Agent Desktop peut soit démarrer son propre serveur local (par défaut), soit se connecter à un serveur URL que vous avez configuré.
...
Si votre `exos-agent.json(c)` contient une section `server`, supprimez-la temporairement et redémarrez l'application de bureau.

#### Vérifier les variables d'environnement

Si `EXOS_AGENT_PORT` est défini dans votre environnement, l'application de bureau tentera d'utiliser ce port pour le serveur local.

- Désactivez `EXOS_AGENT_PORT` (ou choisissez un port libre) et redémarrez.
...
Sur Windows, Exos Agent Desktop nécessite Microsoft Edge **WebView2 Runtime**. Si l'application s'ouvre sur une fenêtre vide ou ne démarre pas, installez/mettez à jour WebView2 et réessayez.

---

### Windows : problèmes de performances généraux

Si vous rencontrez des performances lentes, des problèmes d'accès aux fichiers ou des problèmes de terminal sur Windows, essayez d'utiliser [WSL (Windows Sous-système pour Linux)](/docs/windows-wsl). WSL fournit un environnement Linux qui fonctionne de manière plus transparente avec les fonctionnalités de Exos Agent.

---

### Les notifications ne s'affichent pas

Exos Agent Desktop affiche uniquement les notifications système lorsque :

- les notifications sont activées pour Exos Agent dans les paramètres de votre système d'exploitation, et
...
1. Quittez le bureau Exos Agent.
2. Recherchez et supprimez ces fichiers (ils se trouvent dans le répertoire de données de l'application Exos Agent Desktop) :

- `exos-agent.settings.dat` (serveur de bureau par défaut URL)
- `exos-agent.global.dat` et `exos-agent.workspace.*.dat` (état de l'interface utilisateur comme les serveurs/projets récents)
...
Si vous rencontrez des problèmes avec Exos Agent :
...
3. Assurez-vous d'avoir la dernière version avec `exos-agent upgrade`
...
Pour déterminer à quels modèles vous avez accès, exécutez `exos-agent models`
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.local\share\exos-agent`
...
Si vous rencontrez des erreurs d’appel API, cela peut être dû à des packages de fournisseurs obsolètes. exos-agent installe dynamiquement les packages du fournisseur (OpenAI, Anthropic, Google, etc.) selon les besoins et les met en cache localement.
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.cache\exos-agent`

2. Redémarrez exos-agent pour réinstaller les derniers packages du fournisseur

Cela forcera exos-agent à télécharger les versions les plus récentes des packages du fournisseur, ce qui résout souvent les problèmes de compatibilité avec les paramètres du modèle et les modifications de API.
...
exos-agent détectera si vous utilisez Wayland et préférez `wl-clipboard`, sinon il essaiera de trouver les outils du presse-papiers dans l'ordre : `xclip` et `xsel`.
