---
title: Plugins
description: Écrivez vos propres plugins pour étendre Exos Agent.
---

Les plugins vous permettent d'étendre Exos Agent en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Exos Agent.
...
- `.exos-agent/plugins/` - Plugins au niveau du projet
- `~/.config/exos-agent/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/exos-agent/node_modules/`.
...
1. Configuration globale (`~/.config/exos-agent/exos-agent.json`)
2. Configuration du projet (`exos-agent.json`)
3. Répertoire global des plugins (`~/.config/exos-agent/plugins/`)
4. Répertoire des plugins du projet (`.exos-agent/plugins/`)
...
Exos Agent exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK exos-agent pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre exos-agent.
...
:::note
Si vous utilisez l'application de bureau Exos Agent, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez exos-agent de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à exos-agent :
...
L'assistant `tool` crée un outil personnalisé que exos-agent peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour exos-agent aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://exos-agent.ai/docs/sdk) pour plus de détails.
