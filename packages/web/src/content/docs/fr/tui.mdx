---
title: TUI
description: Utilisation de l’interface utilisateur du terminal Exos Agent.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Exos Agent fournit une interface de terminal interactif ou TUI pour travailler sur vos projets avec un LLM.

L’exécution de Exos Agent démarre le TUI pour le répertoire actuel.
...
Lorsque vous utilisez le Exos Agent TUI, vous pouvez taper `/` suivi d'un nom de commande pour exécuter rapidement des actions. Par exemple:
...
Ajoutez un fournisseur à Exos Agent. Vous permet de sélectionner parmi les fournisseurs disponibles et d’ajouter leurs clés API.
...
Quittez Exos Agent. *Alias* : `/quit`, `/q`
...
Ceci est séparé de `exos-agent.json`, qui configure le comportement du serveur/d'exécution.
...
- `leader_timeout` - Contrôle la durée pendant laquelle Exos Agent attend après la leader key. Valeur par défaut : `2000`.
...
- `sound_pack` - ID du sound pack à utiliser. Valeur par défaut : `exos-agent.default`.
