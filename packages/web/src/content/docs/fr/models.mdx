...
Les fournisseurs les plus populaires sont préchargés par défaut. Si vous avez ajouté les informations d'identification d'un fournisseur via la commande `/connect`, elles seront disponibles lorsque vous démarrerez Exos Agent.
...
Voici plusieurs modèles qui fonctionnent bien avec Exos Agent, sans ordre particulier. (Cette liste n’est pas exhaustive et n’est pas nécessairement à jour) :
...
Pour définir l'un d'entre eux comme modèle par défaut, vous pouvez définir la clé `model` dans votre configuration Exos Agent.

```json title="exos-agent.json" {3}
{
...
Ici, l'identifiant complet est `provider_id/model_id`. Par exemple, si vous utilisez [Exos Agent Zen](/docs/zen), vous utiliserez `exos-agent/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```jsonc title="exos-agent.jsonc" {7-12,19-24}
{
  "$schema": "https://exos-agent.ai/config.json",
  "provider": {
    "openai": {
...
```jsonc title="exos-agent.jsonc" {6-21}
{
  "$schema": "https://exos-agent.ai/config.json",
  "provider": {
    "exos-agent": {
...
De nombreux modèles prennent en charge plusieurs variantes avec différentes configurations. Exos Agent est livré avec des variantes par défaut intégrées pour les fournisseurs populaires.

### Variantes intégrées

Exos Agent est livré avec des variantes par défaut pour de nombreux fournisseurs :
...
```jsonc title="exos-agent.jsonc" {7-18}
{
  "$schema": "https://exos-agent.ai/config.json",
  "provider": {
    "openai": {
...
Lorsque Exos Agent démarre, il recherche les modèles dans l'ordre de priorité suivant :
...
2. La liste des modèles dans la configuration Exos Agent.
