---
title: Serveur
description: Interagissez avec le serveur exos-agent via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `exos-agent serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client exos-agent peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `exos-agent.local` |
...
Définissez `EXOS_AGENT_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `exos-agent`, ou définissez `EXOS_AGENT_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `exos-agent serve` et à `exos-agent web`.
...
Lorsque vous exécutez `exos-agent`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur exos-agent pour interagir avec exos-agent par programmation.
:::

Cette architecture permet à exos-agent de prendre en charge plusieurs clients et vous permet d'interagir avec exos-agent par programme.

Vous pouvez exécuter `exos-agent serve` pour démarrer un serveur autonome. Si vous avez le
exos-agent TUI en cours d'exécution, `exos-agent serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Exos Agent [IDE](/docs/ide).
...
Le serveur exos-agent expose les API suivantes.
