...
Avec Exos Agent, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Exos Agent utilise notre propre thème `exos-agent`.
...
Exos Agent est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Exos Agent corresponde à l'apparence de leur terminal
...
Exos Agent prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/exos-agent/themes/*.json` ou `$XDG_CONFIG_HOME/exos-agent/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.exos-agent/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.exos-agent/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/exos-agent/themes
vim ~/.config/exos-agent/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .exos-agent/themes
vim .exos-agent/themes/my-theme.json
```
...
{
  "$schema": "https://exos-agent.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
