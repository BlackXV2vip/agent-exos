---
title: SDK
description: Client JS de type sécurisé pour le serveur exos-agent.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK exos-agent JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler exos-agent par programme.
...
Créez une instance de exos-agent :

```javascript
import { createExosAgent } from "@exos-agent/sdk"

const { client } = await createExosAgent()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `exos-agent.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${exos-agent.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de exos-agent, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@exos-agent/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*exos-agent" },
})
