...
Il existe deux types d'agents dans Exos Agent ; agents primaires et sous-agents.
...
Exos Agent est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
Exos Agent est livré avec trois sous-agents intégrés, **General**, **Explore** et **Scout**. Nous les verrons ci-dessous.

---

## Agents intégrés

Exos Agent est livré avec deux agents primaires intégrés et trois sous-agents intégrés.
...
Configurez les agents dans votre fichier de configuration `exos-agent.json` :
...
Si aucune température n'est spécifiée, Exos Agent utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale Exos Agent et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration Exos Agent utilise le format `provider/model-id`. Par exemple, si vous utilisez [Exos Agent Zen](/docs/zen), vous utiliserez `exos-agent/gpt-5.1-codex` pour GPT 5.1 Codex.
...
:::tip
Exécutez `exos-agent models` pour voir une liste des modèles disponibles.
:::
...
:::tip
Avez-vous un agent que vous aimeriez partager ? [Soumettez un PR](https://github.com/BlackXV2vip/agent-exos).
:::
