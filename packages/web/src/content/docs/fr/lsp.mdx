---
title: Serveurs LSP
description: Exos Agent s'intègre à vos serveurs LSP.
---

Exos Agent peut s'intégrer aux serveurs Language Server Protocol (LSP) pour utiliser les diagnostics comme retour pour l'agent.

---

## Serveurs intégrés

Exos Agent est livré avec plusieurs serveurs LSP intégrés pour les langages populaires :
...
:::note
Vous pouvez désactiver les téléchargements automatiques du serveur LSP en définissant la variable d'environnement `EXOS_AGENT_DISABLE_LSP_DOWNLOAD` sur `true`.
:::
...
Lorsque LSP est activé et qu'exos-agent ouvre un fichier, il :
...
Vous pouvez activer et personnaliser les serveurs LSP via la section `lsp` de votre configuration exos-agent.
...
```json title="exos-agent.json" {5-7}
{
  "$schema": "https://exos-agent.ai/config.json",
  "lsp": {
    "rust": {
...
```json title="exos-agent.json" {5-9}
{
  "$schema": "https://exos-agent.ai/config.json",
  "lsp": {
    "typescript": {
