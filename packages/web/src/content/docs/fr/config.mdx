---
title: Configuration
description: Utilisation de la configuration JSON Exos Agent.
---

Vous pouvez configurer Exos Agent à l'aide d'un fichier de configuration JSON.

---

## Format

Exos Agent prend en charge les formats **JSON** et **JSONC** (JSON avec commentaires).
...
1. **Configuration distante** (à partir de `.well-known/exos-agent`) - paramètres par défaut de l'organisation
2. **Configuration globale** (`~/.config/exos-agent/exos-agent.json`) - préférences utilisateur
3. **Configuration personnalisée** (`EXOS_AGENT_CONFIG` env var) - remplacements personnalisés
4. **Configuration du projet** (`exos-agent.json` dans le projet) - paramètres spécifiques au projet
5. **Répertoires `.exos-agent`** - agents, commandes, plugins
6. **Configuration en ligne** (`EXOS_AGENT_CONFIG_CONTENT` env var) - remplacements d'exécution
...
:::note
Les répertoires `.exos-agent` et `~/.config/exos-agent` utilisent des **noms au pluriel** pour les sous-répertoires : `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` et `themes/`. Les noms singuliers (par exemple, `agent/`) sont également pris en charge pour une compatibilité ascendante.
:::
...
Les organisations peuvent fournir une configuration par défaut via le point de terminaison `.well-known/exos-agent`. Ceci est récupéré automatiquement lorsque vous vous authentifiez auprès d’un fournisseur qui le prend en charge.
...
Placez votre configuration globale Exos Agent dans `~/.config/exos-agent/exos-agent.json`. Utilisez la configuration globale pour les préférences de l'utilisateur telles que les fournisseurs, les modèles et les autorisations.

Pour les paramètres spécifiques à TUI, utilisez `~/.config/exos-agent/tui.json`.
...
Ajoutez `exos-agent.json` à la racine de votre projet. La configuration du projet a la priorité la plus élevée parmi les fichiers de configuration standard : elle remplace les configurations globales et distantes.
...
Lorsque Exos Agent démarre, il recherche un fichier de configuration dans le répertoire actuel ou remonte jusqu'au répertoire Git le plus proche.
...
Spécifiez un chemin de fichier de configuration personnalisé à l'aide de la variable d'environnement `EXOS_AGENT_CONFIG`.
...
Spécifiez un répertoire de configuration personnalisé à l'aide de `EXOS_AGENT_CONFIG_DIR` variable d'environnement. Ce répertoire sera recherché pour les agents, les commandes, modes et plugins tout comme le répertoire standard `.exos-agent`, et devrait suivre la même structure.
...
Le répertoire personnalisé est chargé après les répertoires de configuration globale et `.exos-agent`, il **peut donc remplacer** leurs paramètres.

---

## Schéma

Le fichier de configuration a un schéma défini dans [**`exos-agent.ai/config.json`**](https://exos-agent.ai/config.json).

La configuration TUI utilise [**`exos-agent.ai/tui.json`**](https://exos-agent.ai/tui.json).
...
Utilisez `EXOS_AGENT_TUI_CONFIG` pour pointer vers un fichier de configuration TUI personnalisé.

Les anciennes clés `theme`, `keybinds` et `tui` dans `exos-agent.json` sont obsolètes et migrées automatiquement lorsque cela est possible.
...
Vous pouvez configurer les paramètres du serveur pour les commandes `exos-agent serve` et `exos-agent web` via l'option `server`.
...
- `mdns` - Activer la découverte du service mDNS. Cela permet à d'autres appareils du réseau de découvrir votre serveur Exos Agent.
- `mdnsDomain` - Nom de domaine personnalisé pour le service mDNS. La valeur par défaut est `exos-agent.local`. Utile pour exécuter plusieurs instances sur le même réseau.
...
Vous pouvez configurer les fournisseurs et les modèles que vous souhaitez utiliser dans votre configuration Exos Agent via les options `provider`, `model` et `small_model`.
...
L'option `small_model` configure un modèle distinct pour les tâches légères comme la génération de titres. Par défaut, Exos Agent essaie d'utiliser un modèle moins cher s'il est disponible auprès de votre fournisseur, sinon il revient à votre modèle principal.
...
Vous pouvez également définir des agents à l'aide de fichiers markdown dans `~/.config/exos-agent/agents/` ou `.exos-agent/agents/`. [En savoir plus ici](/docs/agents).
...
L'agent par défaut doit être un agent principal (et non un sous-agent). Il peut s'agir d'un agent intégré tel que `"build"` ou `"plan"`, ou d'un [agent personnalisé](/docs/agents) que vous avez défini. Si l'agent spécifié n'existe pas ou est un sous-agent, Exos Agent reviendra à `"build"` avec un avertissement.

Ce paramètre s'applique à toutes les interfaces : TUI, CLI (`exos-agent run`), application de bureau et GitHub Action.
...
Vous pouvez également définir des commandes à l'aide de fichiers markdown dans `~/.config/exos-agent/commands/` ou `.exos-agent/commands/`. [En savoir plus ici](/docs/commands).
...
Exos Agent téléchargera automatiquement toutes les nouvelles mises à jour au démarrage. Vous pouvez désactiver cela avec l'option `autoupdate`.
...
Par défaut, exos-agent **autorise toutes les opérations** sans nécessiter d'approbation explicite. Vous pouvez modifier cela en utilisant l'option `permission`.
...
[Plugins](/docs/plugins) étendent Exos Agent avec des outils, des hooks et des intégrations personnalisés.

Placez les fichiers du plugin dans `.exos-agent/plugins/` ou `~/.config/exos-agent/plugins/`. Vous pouvez également charger des plugins depuis npm via l'option `plugin`.
...
Ceci est utile lorsque vous souhaitez restreindre Exos Agent à l'utilisation de fournisseurs spécifiques plutôt que de les désactiver un par un.
