---
title: LSP 伺服器
description: Exos Agent 與您的 LSP 伺服器整合。
---

Exos Agent 可以與語言伺服器協定（LSP）伺服器整合，將診斷資訊作為 agent 的回饋。

---

## 內建支援

Exos Agent 內建了多種適用於主流語言的 LSP 伺服器：
...
:::note
您可以將 `EXOS_AGENT_DISABLE_LSP_DOWNLOAD` 環境變數設定為 `true` 來停用 LSP 伺服器的自動下載。
:::
...
您可以透過 Exos Agent 設定檔中的 `lsp` 部分來啟用並自訂 LSP 伺服器。
