...
There are two types of agents in Exos Agent; primary agents and subagents.
...
Exos Agent comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
Exos Agent comes with three built-in subagents, **General**, **Explore**, and **Scout**. We'll look at this below.

---

## Built-in

Exos Agent comes with two built-in primary agents and three built-in subagents.
...
A read-only agent for external docs and dependency research. Use this when you need to clone a dependency repository into Exos Agent's managed cache, inspect library source, or cross-reference local code against upstream implementations without modifying your workspace.
...
Configure agents in your `exos-agent.json` config file:
...
If no temperature is specified, Exos Agent uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global Exos Agent config and the project specific config.
...
The model ID in your Exos Agent config uses the format `provider/model-id`. For example, if you're using [Exos Agent Zen](/docs/zen), you would use `exos-agent/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Run `exos-agent models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/BlackXV2vip/agent-exos).
:::
