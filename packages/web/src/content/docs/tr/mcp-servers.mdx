...
_Model Bağlam Protokolü_ veya MCP kullanarak exos-agent'a harici araçlar ekleyebilirsiniz. exos-agent hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [exos-agent Config](https://exos-agent.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/exos-agent` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
exos-agent, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde exos-agent şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa exos-agent, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `exos-agent mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından exos-agent, belirteçleri `~/.local/share/exos-agent/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra exos-agent'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi exos-agent yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve exos-agent'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
