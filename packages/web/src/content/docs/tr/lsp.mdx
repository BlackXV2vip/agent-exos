...
description: Exos Agent, LSP sunucularınızla bütünleşir.
---

Exos Agent, tanılamayı agent için geri bildirim olarak kullanmak üzere Language Server Protocol (LSP) sunucularıyla bütünleşebilir.

---

## Yerleşik

Exos Agent, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`EXOS_AGENT_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
LSP etkinleştirildiğinde ve exos-agent bir dosyayı açtığında:
...
LSP sunucularını, exos-agent yapılandırmanızdaki `lsp` bölümü aracılığıyla etkinleştirebilir ve özelleştirebilirsiniz.
