---
title: Özel Araçlar
description: LLM'un exos-agent'da çağırabileceği araçları birleştirebilir.
---

Özel araçlar, LLM'un konuşmalar sırasında arayabileceği, oluşturduğunuz işlevlerdir. exos-agent'un [built-in tools](/docs/tools) ile birlikte `read`, `write` ve `bash` gibi çalışırlar.
...
- Yerel olarak bunları projenizin `.exos-agent/tools/` dizinine yerleştirerek.
- Veya küresel olarak bunları `~/.config/exos-agent/tools/` içine yerleştirerek.
