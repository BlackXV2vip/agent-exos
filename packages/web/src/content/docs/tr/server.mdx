---
title: Sunucu
description: exos-agent sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`exos-agent serve` komutu, exos-agent istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `exos-agent.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `EXOS_AGENT_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `exos-agent` değeridir; değiştirmek isterseniz `EXOS_AGENT_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `exos-agent serve` hem de `exos-agent web` için geçerlidir.
...
`exos-agent` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
exos-agent ile programatik etkileşim için exos-agent sunucusunu kullanın.
:::

Bu mimari, exos-agent'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`exos-agent serve` ile bağımsız bir sunucu başlatabilirsiniz. exos-agent TUI açıksa, `exos-agent serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum exos-agent [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
exos-agent sunucusu aşağıdaki API'leri sunar.
