...
Exos Agent Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, Exos Agent'daki diğer sağlayıcılar gibi çalışır. Exos Agent Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve Exos Agent'u kullanmak için bunu kullanmanıza gerek yoktur.
...
Exos Agent Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

Exos Agent Go, Exos Agent'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>Exos Agent Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `ExosAgent Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye Exos Agent Go'ya abone olabilir.
:::
...
Exos Agent Go aşağıdaki limitleri içerir:
...
Exos Agent yapılandırmanızdaki [model id](/docs/config/#models) formatı `exos-agent-go/<model-id>` şeklindedir. Örneğin, Kimi K3 için yapılandırmanızda `exos-agent-go/kimi-k3` kullanmalısınız.
...
Exos Agent Go'yu şu amaçlarla oluşturduk:
...
4. Exos Agent ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
