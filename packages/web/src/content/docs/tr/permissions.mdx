...
exos-agent, belirli bir eylemin otomatik olarak mı çalıştırılacağına, size sorulacağına veya engelleneceğine karar vermek için `permission` yapılandırmasını kullanır.
...
exos-agent'un başlatıldığı çalışma dizini dışındaki yollara dokunan araç çağrılarına izin vermek için `external_directory` kullanın. Bu, girdi olarak bir yolu alan tüm araçlar için geçerlidir (örneğin `read`, `edit`, `glob`, `grep` ve birçok `bash` komutu).
...
exos-agent izinleri araç adına ve birkaç güvenlik önlemine göre anahtarlanır:
...
Hiçbir şey belirtmezseniz exos-agent izin verilen varsayılanlardan başlar:
...
exos-agent onay istediğinde kullanıcı arayüzü üç sonuç sunar:

- `once` — yalnızca bu isteği onayla
- `always` — önerilen kalıplarla eşleşen gelecekteki istekleri onaylayın (geçerli exos-agent oturumunun geri kalanı için)
