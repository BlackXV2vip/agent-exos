...
`.exos-agent/commands/test.md` oluştur:
...
exos-agent işlemleri aracılığıyla veya `commands/` dizininde işaretleme dosyalarının oluşturulması özel komutlar ile yapılabilir.

---

### JSON

exos-agent [config](/docs/config)'deki `command` seçeneğini kullanın:
...
- Küresel: `~/.config/exos-agent/commands/`
- Proje başına: `.exos-agent/commands/`
...
exos-agent, `/init`, `/undo`, `/redo`, `/share`, `/help` gibi çeşitli yerleşik komutlar içerir; [learn more](/docs/tui#commands).
