---
title: Sağlayıcılar
description: exos-agent'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

exos-agent, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı exos-agent yapılandırmanızda yapılandırın.
...
`~/.local/share/exos-agent/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları exos-agent'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
Exos Agent Zen, exos-agent ekibi tarafından sağlanan modellerin bir listesidir.
exos-agent ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz Exos Agent Zen ile başlamanızı öneririz.
:::
...
exos-agent'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## Exos Agent Go

Exos Agent Go, exos-agent ile iyi çalıştığı test edilmiş ve doğrulanmış, exos-agent ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `ExosAgent Go`'yu seçin ve [exos-agent.ai/auth](https://exos-agent.ai/zen) adresine gidin.
...
Amazon Bedrock'u exos-agent ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   exos-agent'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `exos-agent.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi exos-agent'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
exos-agent'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   exos-agent'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   Exos Agent yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
:::note[uyumluluk notu]
exos-agent, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. exos-agent'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`exos-agent.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [exos-agent-gitlab-auth](https://www.npmjs.com/package/exos-agent-gitlab-auth) ana sayfasında.

##### Yapılandırma

`exos-agent.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi exos-agent'la kullanmak için:
...
Google Vertex AI'yi exos-agent ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları exos-agent çalıştırılırken ayarlayın.
...
Helicone'un exos-agent aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her exos-agent görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [exos-agent-helicone-session](https://github.com/H2Shami/exos-agent-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her exos-agent görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
exos-agent'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla exos-agent'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla exos-agent'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini exos-agent için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/exos-agent)'a bakın.
:::
...
Ollama Cloud'u exos-agent ile kullanmak için:
...
3. exos-agent'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: exos-agent'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
Exos Agent Zen, exos-agent ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>Exos Agent Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **Exos Agent Zen**'i arayın.

   ```txt
   /connect
   ```

3. exos-agent API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   exos-agent yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca exos-agent yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi Exos Agent ile kullanmak için:
...
Modelleri exos-agent yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı exos-agent ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `exos-agent.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları exos-agent'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `exos-agent auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için exos-agent yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, exos-agent yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
