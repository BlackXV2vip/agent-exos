...
Ajan becerileri, exos-agent'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
exos-agent su konumlari tarar:

- Proje konfigurasyonu: `.exos-agent/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/exos-agent/skills/<name>/SKILL.md`
...
Proje yerel yollarinda exos-agent, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.exos-agent/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/exos-agent/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.exos-agent/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
exos-agent, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`exos-agent.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`exos-agent.json` icinde):
