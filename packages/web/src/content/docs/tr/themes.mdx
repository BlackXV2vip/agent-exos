...
exos-agent ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak exos-agent kendi `exos-agent` temasını kullanır.
...
exos-agent birden fazla yerleşik temayla gelir.
...
- exos-agent'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
exos-agent, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/exos-agent/themes/*.json` veya `$XDG_CONFIG_HOME/exos-agent/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.exos-agent/themes/*.json`
4. **Geçerli çalışma dizini** - `./.exos-agent/themes/*.json`
