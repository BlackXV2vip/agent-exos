...
exos-agent'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
exos-agent, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
Exos Agent, **General**, **Explore** ve **Scout** olmak üzere üç yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

Exos Agent iki yerleşik birincil agent ve üç yerleşik alt agent ile birlikte gelir.
...
Agent'ları `exos-agent.json` yapılandırma dosyanızda yapılandırın:
...
- Proje başına: `.exos-agent/agents/`
...
Sıcaklık belirtilmezse exos-agent modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global exos-agent yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
exos-agent yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [Exos Agent Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `exos-agent/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `exos-agent models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/BlackXV2vip/agent-exos).
:::
