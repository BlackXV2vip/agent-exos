---
title: Servidor
description: Interaja com o servidor exos-agent via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `exos-agent serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente exos-agent pode usar.
...
```bash
exos-agent serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `exos-agent.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
exos-agent serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `EXOS_AGENT_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `exos-agent`, ou defina `EXOS_AGENT_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `exos-agent serve` quanto ao `exos-agent web`.

```bash
EXOS_AGENT_SERVER_PASSWORD=your-password exos-agent serve
```
...
Quando você executa `exos-agent`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor exos-agent para interagir com o exos-agent programaticamente.
:::

Essa arquitetura permite que o exos-agent suporte múltiplos clientes e permite que você interaja com o exos-agent programaticamente.

Você pode executar `exos-agent serve` para iniciar um servidor autônomo. Se você tiver o TUI do exos-agent em execução, `exos-agent serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do exos-agent [IDE](/docs/ide).
...
O servidor exos-agent expõe as seguintes APIs.
