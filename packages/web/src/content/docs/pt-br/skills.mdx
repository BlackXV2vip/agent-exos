...
As habilidades do agente permitem que o exos-agent descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O exos-agent pesquisa nesses locais:

- Configuração do projeto: `.exos-agent/skills/<name>/SKILL.md`
- Configuração global: `~/.config/exos-agent/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o exos-agent sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.exos-agent/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/exos-agent/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.exos-agent/skills/git-release/SKILL.md` assim:
...
O exos-agent lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `exos-agent.json`:
...
**Para agentes embutidos** (em `exos-agent.json`):
