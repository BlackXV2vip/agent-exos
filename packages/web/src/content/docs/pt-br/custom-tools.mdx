...
description: Crie ferramentas que o LLM pode chamar no exos-agent.
---

Ferramentas personalizadas são funções que você cria e que o LLM pode chamar durante as conversas. Elas funcionam junto com as [ferramentas integradas](/docs/tools) do exos-agent, como `read`, `write` e `bash`.
...
- Localmente, colocando-as no diretório `.exos-agent/tools/` do seu projeto.
- Ou globalmente, colocando-as em `~/.config/exos-agent/tools/`.
