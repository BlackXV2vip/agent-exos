---
title: Formatadores
description: O exos-agent usa formatadores específicos de linguagem.
---

O exos-agent formata automaticamente arquivos após serem escritos ou editados usando formatadores específicos de linguagem. Isso garante que o código gerado siga os estilos de código do seu projeto.

---

## Integrados

O exos-agent vem com vários formatadores integrados para linguagens e frameworks populares. Abaixo está uma lista dos formatadores, extensões de arquivo suportadas e comandos ou opções de configuração necessárias.
...
Portanto, se seu projeto tiver `prettier` em seu `package.json`, o exos-agent o usará automaticamente.

---

## Como funciona

Quando o exos-agent escreve ou edita um arquivo, ele:
...
Você pode personalizar os formatadores através da seção `formatter` em sua configuração do exos-agent.
