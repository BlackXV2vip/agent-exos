...
Com o exos-agent, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o exos-agent usa nosso próprio tema `exos-agent`.
...
O exos-agent vem com vários temas embutidos.
...
- Querem que o exos-agent corresponda à aparência do seu terminal
...
O exos-agent suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/exos-agent/themes/*.json` ou `$XDG_CONFIG_HOME/exos-agent/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.exos-agent/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.exos-agent/themes/*.json`
