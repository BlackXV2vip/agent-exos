...
O Exos Agent Go é uma assinatura de baixo custo — **US$ 5 no seu primeiro mês**, depois **US$ 10/mês** — que oferece acesso confiável a modelos abertos de programação populares.

O Go funciona como qualquer outro provedor no Exos Agent. Você assina o Exos Agent Go e
...
O Exos Agent Go lhe dá acesso a esses modelos por **US$ 5 no seu primeiro mês**, depois **US$ 10/mês**.

---

## Como funciona

O Exos Agent Go funciona como qualquer outro provedor no Exos Agent.

1. Você entra no **<a href={console}>Exos Agent Zen</a>**, assina o Go e
   copia a sua chave de API.
2. Você executa o comando `/connect` na TUI, seleciona `ExosAgent Go` e cola
   a sua chave de API.
3. Execute `/models` na TUI para ver a lista de modelos disponíveis através do Go.

:::note
Apenas um membro por workspace pode assinar o Exos Agent Go.
:::
...
O Exos Agent Go inclui os seguintes limites:
...
O [ID do modelo](/docs/config/#models) na sua configuração do Exos Agent
usa o formato `exos-agent-go/<model-id>`. Por exemplo, para o Kimi K3, você usaria
`exos-agent-go/kimi-k3` na sua configuração.
...
4. Não ter **nenhuma restrição de fornecedor (lock-in)**, permitindo que você use qualquer outro provedor com o Exos Agent também.
