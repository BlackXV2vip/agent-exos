---
title: Plugins
description: Escreva seus próprios plugins para estender o exos-agent.
---

Plugins permitem que você estenda o exos-agent conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do exos-agent.
...
- `.exos-agent/plugins/` - Plugins em nível de projeto
- `~/.config/exos-agent/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/exos-agent/node_modules/`.
...
1. Configuração global (`~/.config/exos-agent/exos-agent.json`)
2. Configuração do projeto (`exos-agent.json`)
3. Diretório de plugins global (`~/.config/exos-agent/plugins/`)
4. Diretório de plugins do projeto (`.exos-agent/plugins/`)
...
O exos-agent executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do exos-agent para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o exos-agent.
...
:::note
Se você estiver usando o aplicativo desktop exos-agent, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o exos-agent de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao exos-agent:
...
O helper `tool` cria uma ferramenta personalizada que o exos-agent pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o exos-agent junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://exos-agent.ai/docs/sdk) para detalhes.
