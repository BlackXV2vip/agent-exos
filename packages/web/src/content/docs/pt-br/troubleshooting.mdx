...
Para depurar problemas com o exos-agent, comece verificando os logs e os dados locais que ele armazena no disco.
...
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\exos-agent\log`
...
Você pode definir o nível de log com a opção de linha de comando `--log-level` para obter informações de depuração mais detalhadas. Por exemplo, `exos-agent --log-level DEBUG`.

---

## Armazenamento

O exos-agent armazena dados de sessão e outros dados do aplicativo no disco em:

- **macOS/Linux**: `~/.local/share/exos-agent/`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\exos-agent`
...
O exos-agent Desktop executa um servidor exos-agent local (o sidecar `exos-agent-cli`) em segundo plano. A maioria dos problemas é causada por um plugin com mau funcionamento, um cache corrompido ou uma configuração de servidor incorreta.
...
- Apenas macOS: menu `ExosAgent` -> **Recarregar Webview** (ajuda se a interface estiver em branco/congelada).
...
- **macOS/Linux**: `~/.config/exos-agent/exos-agent.jsonc` (ou `~/.config/exos-agent/exos-agent.json`)
- **macOS/Linux** (instalações mais antigas): `~/.local/share/exos-agent/exos-agent.jsonc`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\exos-agent\exos-agent.jsonc`
...
O exos-agent também pode carregar plugins locais do disco. Mova-os temporariamente para fora do caminho (ou renomeie a pasta) e reinicie o aplicativo de desktop:

- **Plugins globais**
  - **macOS/Linux**: `~/.config/exos-agent/plugins/`
  - **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\exos-agent\plugins`
...
Se desativar plugins não ajudar (ou se a instalação de um plugin estiver travada), limpe o cache para que o exos-agent possa reconstruí-lo.

1. Saia completamente do exos-agent Desktop.
2. Exclua o diretório de cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> cole `~/.cache/exos-agent`
- **Linux**: exclua `~/.cache/exos-agent` (ou execute `rm -rf ~/.cache/exos-agent`)
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.cache\exos-agent`

3. Reinicie o exos-agent Desktop.

---

### Corrigindo problemas de conexão com o servidor

O exos-agent Desktop pode iniciar seu próprio servidor local (padrão) ou conectar-se a uma URL de servidor que você configurou.
...
Se seu `exos-agent.json(c)` contiver uma seção `server`, remova-a temporariamente e reinicie o aplicativo de desktop.

#### Verificando as variáveis de ambiente

Se você tiver `EXOS_AGENT_PORT` definido em seu ambiente, o aplicativo de desktop tentará usar essa porta para o servidor local.

- Desfaça `EXOS_AGENT_PORT` (ou escolha uma porta livre) e reinicie.
...
No Windows, o exos-agent Desktop requer o **WebView2 Runtime** do Microsoft Edge. Se o aplicativo abrir em uma janela em branco ou não iniciar, instale/atualize o WebView2 e tente novamente.

---

### Windows: Problemas gerais de desempenho

Se você estiver enfrentando desempenho lento, problemas de acesso a arquivos ou problemas no terminal no Windows, tente usar [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). O WSL fornece um ambiente Linux que funciona de forma mais integrada com os recursos do exos-agent.

---

### Notificações não aparecendo

O exos-agent Desktop só mostra notificações do sistema quando:

- as notificações estão habilitadas para o exos-agent nas configurações do seu sistema operacional, e
...
1. Saia do exos-agent Desktop.
2. Encontre e exclua estes arquivos (eles estão no diretório de dados do aplicativo exos-agent Desktop):

- `exos-agent.settings.dat` (URL do servidor padrão do desktop)
- `exos-agent.global.dat` e `exos-agent.workspace.*.dat` (estado da interface como servidores/projetos recentes)
...
Se você estiver enfrentando problemas com o exos-agent:
...
Para descobrir quais modelos você tem acesso, execute `exos-agent models`
...
2. Se o problema persistir, tente limpar sua configuração armazenada:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.local\share\exos-agent`
...
Se você encontrar erros de chamada de API, isso pode ser devido a pacotes de provedores desatualizados. O exos-agent instala dinamicamente pacotes de provedores (OpenAI, Anthropic, Google, etc.) conforme necessário e os armazena em cache localmente.
...
1. Limpe o cache do pacote de provedores:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.cache\exos-agent`

2. Reinicie o exos-agent para reinstalar os pacotes de provedores mais recentes

Isso forçará o exos-agent a baixar as versões mais recentes dos pacotes de provedores, o que muitas vezes resolve problemas de compatibilidade com parâmetros de modelo e alterações na API.
...
O exos-agent detectará se você está usando Wayland e preferirá `wl-clipboard`, caso contrário, tentará encontrar ferramentas de área de transferência na ordem: `xclip` e `xsel`.
