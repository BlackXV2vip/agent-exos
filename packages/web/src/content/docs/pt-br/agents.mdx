...
Existem dois tipos de agentes no exos-agent; agentes primários e subagentes.
...
exos-agent vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
Exos Agent vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

Exos Agent vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do Exos Agent, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Configure os agentes em seu arquivo de configuração `exos-agent.json`:
...
- Global: `~/.config/exos-agent/agents/`
- Por projeto: `.exos-agent/agents/`

```markdown title="~/.config/exos-agent/agents/review.md"
---
...
Se nenhuma temperatura for especificada, o exos-agent usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do exos-agent quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do exos-agent usa o formato `provider/model-id`. Por exemplo, se você estiver usando [Exos Agent Zen](/docs/zen), você usaria `exos-agent/gpt-5.1-codex` para GPT 5.1 Codex.
...
```json title="exos-agent.json" {3-6,9-12}
{
  "$schema": "https://exos-agent.ai/config.json",
  "tools": {
    "write": true,
...
```json title="exos-agent.json" {3-5,8-10}
{
  "$schema": "https://exos-agent.ai/config.json",
  "permission": {
    "edit": "deny"
...
```json title="exos-agent.json" {6,7}
{
  "agent": {
    "deep-thinker": {
...
:::tip
Execute `exos-agent models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/BlackXV2vip/agent-exos).
:::
...
```markdown title="~/.config/exos-agent/agents/docs-writer.md"
---
...
```markdown title="~/.config/exos-agent/agents/security-auditor.md"
---
