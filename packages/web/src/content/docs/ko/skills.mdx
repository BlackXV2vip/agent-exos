...
Agent Skill let exos-agent discover reusable instruction from your repo 또는 홈 디렉토리.
...
- 프로젝트 구성: `.exos-agent/skills/<name>/SKILL.md`
- 글로벌 구성: `~/.config/exos-agent/skills/<name>/SKILL.md`
...
Project-local paths의 경우, exos-agent는 git worktree에 도달 할 때까지 현재 작업 디렉토리에서 걷습니다.
그것은 `skills/*/SKILL.md`에 있는 어떤 어울리는 `.exos-agent/` 및 어떤 어울리는 `.claude/skills/*/SKILL.md` 또는 `.agents/skills/*/SKILL.md`를 방법 적재합니다.

세계적인 정의는 또한 `~/.config/exos-agent/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` 및 `~/.agents/skills/*/SKILL.md`에서 적재됩니다.
...
이처럼 `.exos-agent/skills/git-release/SKILL.md` 만들기:
...
exos-agent는 `skill` 도구 설명에서 사용할 수있는 기술을 나열합니다.
...
기술 에이전트가 `exos-agent.json`의 패턴 기반 권한을 사용하여 액세스 할 수있는 제어 :
