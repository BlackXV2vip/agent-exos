---
title: TUI
description: Using the Exos Agent terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Exos Agent provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running Exos Agent starts the TUI for the current directory.
...
When using the Exos Agent TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to Exos Agent. Allows you to select from available providers and add their API keys.
...
Exit Exos Agent. _Aliases_: `/quit`, `/q`
...
This is separate from `exos-agent.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long Exos Agent waits after the leader key. Defaults to `2000`.
...
Use `EXOS_AGENT_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `exos-agent.default`.
