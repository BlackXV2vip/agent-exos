...
Mit Exos Agent kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet Exos Agent das `exos-agent`-Theme.
...
Exos Agent liefert mehrere eingebaute Themes mit.
...
- Exos Agent optisch ans Terminal anpassen wollen
...
Exos Agent unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/exos-agent/themes/*.json` oder `$XDG_CONFIG_HOME/exos-agent/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.exos-agent/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.exos-agent/themes/*.json`
