...
Sie können externe Tools zu Exos Agent hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. Exos Agent unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [Exos Agent Config](https://exos-agent.ai/docs/config/) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.
...
Organisationen können über ihren `.well-known/exos-agent`-Endpunkt Standard-MCP-Server bereitstellen. Diese Server sind möglicherweise standardmäßig deaktiviert, sodass Benutzer sich für den Server entscheiden können, die sie benötigen.
...
Exos Agent übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird Exos Agent Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von Exos Agent beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `exos-agent mcp auth <server-name>` verwenden.
...
Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert Exos Agent die Token sicher in `~/.local/share/exos-agent/mcp-auth.json`.
...
Ihre MCPs sind neben integrierten Tools auch als Tools in Exos Agent verfügbar. Sie können sie also wie jedes andere Tool über die Exos Agent-Konfiguration verwalten.
...
Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und Exos Agent mit Ihrem Sentry-Konto zu verbinden.
