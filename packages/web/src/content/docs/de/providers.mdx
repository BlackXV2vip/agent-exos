---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in Exos Agent.
---

import config from "../../../../config.mjs"
export const console = config.console

Exos Agent verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer Exos Agent-Konfiguration.
...
Sie können die Anbieter über den Abschnitt `provider` in Ihrem Exos Agent anpassen
config.
...
Exos Agent Zen ist eine Liste von Modellen, die vom Exos Agent-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit Exos Agent funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit Exos Agent Zen zu beginnen.
:::
...
Es funktioniert wie jeder andere Anbieter in Exos Agent und ist völlig optional.

---

## Exos Agent Go

Exos Agent Go ist ein kostenguenstiges Abonnement, das zuverlaessigen Zugriff auf beliebte Open-Coding-Modelle bietet, die vom Exos Agent-Team getestet und verifiziert wurden, dass sie gut mit Exos Agent funktionieren.

1. Führen Sie den Befehl `/connect` in der TUI aus, waehlen Sie `ExosAgent Go` und gehen Sie zu [exos-agent.ai/auth](https://exos-agent.ai/zen).
...
So verwenden Sie Amazon Bedrock mit Exos Agent:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie Exos Agent ausführen:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `exos-agent.json`:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in Exos Agent wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
Sie können exos-agent so konfigurieren, dass es lokale Modelle über [Atomic Chat](https://atomic.chat) verwendet — eine Desktop-Anwendung, die lokale LLMs hinter einem OpenAI-kompatiblen API-Server bereitstellt (Standard-Endpunkt `http://127.0.0.1:1337/v1`).
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit Exos Agent ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre Exos Agent-Konfiguration hinzufügen.
...
um das Token im Exos Agent-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
Exos Agent verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von Exos Agent
Um nur Ihre eigene von GitLab gehostete Instanz zu verwenden, fügen Sie Folgendes zu Ihrer hinzu
`exos-agent.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
Weitere Dokumentation auf der [exos-agent-gitlab-auth](https://www.npmjs.com/package/exos-agent-gitlab-auth)-Homepage.

##### Konfiguration

Anpassen über `exos-agent.json`:
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit Exos Agent:
...
So verwenden Sie Google Vertex AI mit Exos Agent:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie Exos Agent ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über Exos Agent konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [exos-agent-helicone-session](https://github.com/H2Shami/exos-agent-helicone-session)-Plugin, um jede Exos Agent-Konversation automatisch als Sitzung in Helicone zu protokollieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede Exos Agent-Konversation als separate Sitzung aufgeführt.
...
Sie können Exos Agent für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können Exos Agent über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können Exos Agent über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für Exos Agent konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/exos-agent).
:::
...
So verwenden Sie Ollama Cloud mit Exos Agent:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in Exos Agent.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in Exos Agent verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
Exos Agent Zen ist eine Liste getesteter und verifizierter Modelle, die vom Exos Agent-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>Exos Agent Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **Exos Agent Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren Exos Agent API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre Exos Agent-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre Exos Agent-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit Exos Agent:
...
Sie können Modelle auch über Ihre Exos Agent-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit Exos Agent verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
4. Erstellen oder aktualisieren Sie Ihre `exos-agent.json`-Datei in Ihrem Projektverzeichnis:
...
Mithilfe der `limit`-Felder kann Exos Agent erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `exos-agent auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die Exos Agent-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer Exos Agent-Konfiguration übereinstimmt.
