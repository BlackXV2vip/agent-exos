...
Wenn Exos Agent Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\exos-agent\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `exos-agent --log-level DEBUG`.

---

## Speicher

exos-agent speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/exos-agent/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\exos-agent`
...
Exos Agent Desktop startet im Hintergrund einen lokalen Exos Agent-Server (`exos-agent-cli`-Sidecar).
...
- Nur macOS: `ExosAgent`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/exos-agent/exos-agent.jsonc` (oder `~/.config/exos-agent/exos-agent.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/exos-agent/exos-agent.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\exos-agent\exos-agent.jsonc`
...
Exos Agent kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/exos-agent/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\exos-agent\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit Exos Agent ihn neu aufbauen kann.

1. Beende Exos Agent Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/exos-agent`
- **Linux**: loesche `~/.cache/exos-agent` (oder fuehre aus: `rm -rf ~/.cache/exos-agent`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\exos-agent`

3. Starte Exos Agent Desktop neu.

---

### Server-Verbindungsprobleme beheben

Exos Agent Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `exos-agent.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `EXOS_AGENT_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `EXOS_AGENT_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt Exos Agent Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit Exos Agent-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

Exos Agent Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer Exos Agent in den OS-Einstellungen aktiviert sind, und
...
1. Beende Exos Agent Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von Exos Agent Desktop):

- `exos-agent.settings.dat` (desktop default server URL)
- `exos-agent.global.dat` und `exos-agent.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit Exos Agent hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `exos-agent models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\exos-agent`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. exos-agent installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\exos-agent`

2. Starte exos-agent neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt exos-agent, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
exos-agent erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
