---
title: TUI
description: Arbeite mit Exos Agent in der Terminal-Oberflaeche.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Exos Agent bietet eine interaktive Terminaloberflaeche (TUI), um mit einem LLM an deinen Projekten zu arbeiten.

Ohne weitere Argumente startet Exos Agent die TUI im aktuellen Verzeichnis.
...
In der Exos Agent-TUI kannst du `/` plus Befehlsnamen eingeben, um Aktionen schnell auszufuehren.
Zum Beispiel:
...
Fuegt einen Provider zu Exos Agent hinzu. Erlaubt die Auswahl aus verfuegbaren Providern und das Hinzufuegen ihrer API-Keys.
...
Beendet Exos Agent. _Aliase_: `/quit`, `/q`
...
Dies ist getrennt von `exos-agent.json`, welche das Server-/Runtime-Verhalten konfiguriert.
...
- `leader_timeout` - Steuert, wie lange Exos Agent nach der leader key wartet. Standard ist `2000`.
...
Verwende `EXOS_AGENT_TUI_CONFIG`, um einen benutzerdefinierten TUI-Konfigurationspfad zu laden.

### Attention

Attention lässt das TUI dich benachrichtigen, wenn Exos Agent auf eine Antwort wartet, eine Berechtigung bestätigt werden muss, ein Sitzungsfehler aufgetreten ist oder eine Sitzung abgeschlossen wurde. Aktiviere es mit `attention.enabled`; integrierte Ereignisse spielen beim Auftreten einen Sound ab. Desktop-Benachrichtigungen werden nur gesendet, wenn das Terminalfenster nicht fokussiert ist, und nicht für subagent-Ereignisse verwendet.
...
- `sound_pack` - Zu verwendende sound pack ID. Standard ist `exos-agent.default`.
