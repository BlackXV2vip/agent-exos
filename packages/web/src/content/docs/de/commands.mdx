...
Erstellen Sie `.exos-agent/commands/test.md`:

```md title=".exos-agent/commands/test.md"
---
...
Sie können benutzerdefinierte Befehle über die Exos Agent-Konfiguration hinzufügen oder indem Sie Markdown-Dateien im Verzeichnis `commands/` erstellen.

---

### JSON

Verwenden Sie die Option `command` in Ihrem Exos Agent [config](/docs/config):

```json title="exos-agent.jsonc" {4-12}
{
...
- Global: `~/.config/exos-agent/commands/`
- Pro Projekt: `.exos-agent/commands/`

```markdown title="~/.config/exos-agent/commands/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
```md title=".exos-agent/commands/component.md"
---
...
```md title=".exos-agent/commands/create-file.md"
---
...
```md title=".exos-agent/commands/analyze-coverage.md"
---
...
```md title=".exos-agent/commands/review-changes.md"
---
...
```md title=".exos-agent/commands/review-component.md"
---
...
Exos Agent enthält mehrere integrierte Befehle wie `/init`, `/undo`, `/redo`, `/share`, `/help`; [mehr erfahren](/docs/tui#commands).
