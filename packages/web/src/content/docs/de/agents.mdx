...
Es gibt zwei Arten von Agenten in Exos Agent; Primäragenten und Subagenten.
...
Exos Agent verfügt über zwei integrierte Primäragenten: **Build** und **Plan**. Auch
...
Konfigurieren Sie Agenten in Ihrer `exos-agent.json`-Konfigurationsdatei:
...
Wenn keine Temperatur angegeben ist, verwendet Exos Agent modellspezifische Standardwerte; typischerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle.
...
Dieser Pfad ist relativ zum Speicherort der Konfigurationsdatei. Dies funktioniert sowohl für die globale Exos Agent-Konfiguration als auch für die projektspezifische Konfiguration.
...
Die Modell-ID in Ihrer Exos Agent-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [Exos Agent Zen](/docs/zen) verwenden, würden Sie `exos-agent/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
...
:::tip
Führen Sie `exos-agent models` aus, um eine Liste der verfügbaren Modelle anzubieten.
:::
...
:::tip
Haben Sie einen Agenten, den Sie gerne teilen möchten? [Reichen Sie einen PR ein](https://github.com/BlackXV2vip/agent-exos).
:::
