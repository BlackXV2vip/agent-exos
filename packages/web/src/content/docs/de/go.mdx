...
Exos Agent Go ist ein kostengünstiges Abonnement — **5 $ für deinen ersten Monat**, danach **10 $/Monat** —, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

Go funktioniert wie jeder andere Provider in Exos Agent. Du abonnierst Exos Agent Go und
erhältst deinen API-Key. Es ist **völlig optional** und du musst es nicht nutzen, um
Exos Agent zu verwenden.
...
Exos Agent Go bietet dir Zugriff auf diese Modelle für **5 $ im ersten Monat**, danach **10 $/Monat**.

---

## Wie es funktioniert

Exos Agent Go funktioniert wie jeder andere Provider in Exos Agent.

1. Du meldest dich bei **<a href={console}>Exos Agent Zen</a>** an, abonnierst Go und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `ExosAgent Go` und fügst deinen API-Key ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann Exos Agent Go abonnieren.
:::
...
Exos Agent Go beinhaltet die folgenden Limits:
...
Die [Modell-ID](/docs/config/#models) in deiner Exos Agent Config verwendet das Format `exos-agent-go/<model-id>`. Für Kimi K3 würdest du beispielsweise `exos-agent-go/kimi-k3` in deiner Config verwenden.
...
Wir haben Exos Agent Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit Exos Agent zu nutzen.
