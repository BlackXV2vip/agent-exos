---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Exos Agent zu erweitern.
---

Mit Plugins können Sie Exos Agent erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Exos Agent zu ändern.
...
- `.exos-agent/plugins/` – Plugins auf Projektebene
- `~/.config/exos-agent/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/exos-agent/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/exos-agent/exos-agent.json`)
2. Projektkonfiguration (`exos-agent.json`)
3. Globales Plugin-Verzeichnis (`~/.config/exos-agent/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.exos-agent/plugins/`)
...
Exos Agent führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein Exos Agent-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Exos Agent erweitern können.
...
:::note
Wenn Sie die Exos Agent-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Exos Agent `.env`-Dateien liest:
...
Plugins können Exos Agent auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Exos Agent aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Exos Agent zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://exos-agent.ai/docs/sdk).
