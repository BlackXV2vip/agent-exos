---
title: Regeln
description: Lege eigene Anweisungen fuer exos-agent fest.
---

Du kannst exos-agent ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in exos-agent den Befehl `/init` aus.
...
So kann exos-agent sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht exos-agent sie zu erweitern.
...
exos-agent kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/exos-agent/AGENTS.md` speichern.
Diese gelten in allen exos-agent-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt Exos Agent dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/exos-agent/AGENTS.md` existiert)
...
Beim Start sucht exos-agent in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/exos-agent/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `exos-agent.json` oder in `~/.config/exos-agent/exos-agent.json` angeben.
...
exos-agent parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit exos-agent.json

Empfohlen ist das `instructions`-Feld in der `exos-agent.json`:
...
Du kannst exos-agent explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `exos-agent.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
