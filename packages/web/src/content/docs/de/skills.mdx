...
Agent Skills erlauben Exos Agent, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Exos Agent sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Exos Agent vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.exos-agent/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/exos-agent/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.exos-agent/skills/git-release/SKILL.md` zum Beispiel so:
...
Exos Agent listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `exos-agent.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `exos-agent.json`):
