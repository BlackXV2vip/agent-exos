---
title: Server
description: Interact with exos-agent server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `exos-agent serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an exos-agent client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `exos-agent.local` |
...
Set `EXOS_AGENT_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `exos-agent`, or set `EXOS_AGENT_SERVER_USERNAME` to override it. This applies to both `exos-agent serve` and `exos-agent web`.
...
When you run `exos-agent` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the exos-agent server to interact with exos-agent programmatically.
:::

This architecture lets exos-agent support multiple clients and allows you to interact with exos-agent programmatically.

You can run `exos-agent serve` to start a standalone server. If you have the
exos-agent TUI running, `exos-agent serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Exos Agent [IDE](/docs/ide) plugins.
...
The exos-agent server exposes the following APIs.
