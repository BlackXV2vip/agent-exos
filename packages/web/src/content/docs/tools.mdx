...
Tools allow the LLM to perform actions in your codebase. Exos Agent comes with a set of built-in tools, but you can extend it with [custom tools](/docs/custom-tools) or [MCP servers](/docs/mcp-servers).
...
Here are all the built-in tools available in Exos Agent.
...
:::note
This tool is only available when `EXOS_AGENT_EXPERIMENTAL_LSP_TOOL=true` (or `EXOS_AGENT_EXPERIMENTAL=true`).
:::
...
:::note
This tool is only available when using the Exos Agent provider or when the `EXOS_AGENT_ENABLE_EXA` environment variable is set to any truthy value (e.g., `true` or `1`).

To enable when launching Exos Agent:
