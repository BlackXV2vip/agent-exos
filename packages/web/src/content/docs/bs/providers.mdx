---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Exos Agent.
---

import config from "../../../../config.mjs"
export const console = config.console

Exos Agent koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Exos Agent konfiguraciji.
...
u `~/.local/share/exos-agent/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Exos Agent
config.
...
Exos Agent Zen je lista modela koje je obezbedio Exos Agent tim koji su bili
testirano i potvrđeno da dobro radi sa Exos Agent. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Exos Agent Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite exos-agent i idite na [exos-agent.ai/auth](https://exos-agent.ai/auth).
...
Radi kao i svaki drugi provajder u Exos Agent i potpuno je opcionalan za korištenje.

---

## Exos Agent Go

Exos Agent Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Exos Agent tim i koji su testirani i verificirani da dobro rade s Exos Agent-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `ExosAgent Go` i idite na [exos-agent.ai/auth](https://exos-agent.ai/zen).
...
Da biste koristili Amazon Bedrock s Exos Agent:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete exos-agent:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `exos-agent.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Exos Agent.
:::
...
Možete konfigurirati exos-agent za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi exos-agent ispravno radio.
   :::
...
Također možete dodati modele kroz svoju exos-agent konfiguraciju.
...
da pohrani token u exos-agent auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Exos Agent koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Exos Agent
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`exos-agent.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [exos-agent-gitlab-auth](https://www.npmjs.com/package/exos-agent-gitlab-auth).

##### Konfiguracija

Prilagodite putem `exos-agent.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s exos-agent:
...
Za korištenje Google Vertex AI s Exos Agent:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće exos-agent.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem exos-agenta, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [exos-agent-helicone-session](https://github.com/H2Shami/exos-agent-helicone-session) da automatski prijavite svaki Exos Agent razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Exos Agent razgovor naveden kao zasebna sesija.
...
Možete konfigurirati exos-agent za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati exos-agent za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati exos-agent za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Exos Agent. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/exos-agent) za detalje.
:::
...
Da biste koristili Ollama Cloud s Exos Agent:
...
3. Kopirajte API ključ za korištenje u Exos Agent.
...
6. **Važno**: Prije upotrebe modela oblaka u Exos Agent, morate lokalno povući informacije o modelu:
...
Exos Agent Zen je lista testiranih i verifikovanih modela koju je obezbedio Exos Agent tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Exos Agent Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Exos Agent Zen**.

```txt
   /connect
```

3. Unesite svoj Exos Agent API ključ.
...
Također možete dodati dodatne modele putem vaše exos-agent konfiguracije.
...
5. Također ih možete prilagoditi putem vaše exos-agent konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa Exos Agentom:
...
Također možete prilagoditi modele kroz svoju exos-agent konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s exos-agent-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `exos-agent.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Exos Agent da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `exos-agent auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Exos Agent konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj exos-agent konfiguraciji.
