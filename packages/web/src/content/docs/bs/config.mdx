---
title: Konfiguracija
description: Korištenje Exos Agent JSON konfiguracije.
---

Možete konfigurirati Exos Agent koristeći JSON konfiguracijski fajl.

---

## Format

Exos Agent podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/exos-agent`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/exos-agent/exos-agent.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`EXOS_AGENT_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`exos-agent.json` u projektu) - postavke specifične za projekat
5. **`.exos-agent` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`EXOS_AGENT_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.exos-agent` i `~/.config/exos-agent` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/exos-agent` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu Exos Agent konfiguraciju u `~/.config/exos-agent/exos-agent.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/exos-agent/tui.json`.
...
Dodajte `exos-agent.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se Exos Agent pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `EXOS_AGENT_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `EXOS_AGENT_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.exos-agent` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.exos-agent`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`exos-agent.ai/config.json`**](https://exos-agent.ai/config.json).

TUI konfiguracija koristi [**`exos-agent.ai/tui.json`**](https://exos-agent.ai/tui.json).
...
Koristite `EXOS_AGENT_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `exos-agent.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `exos-agent serve` i `exos-agent web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš Exos Agent server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `exos-agent.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj Exos Agent konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, Exos Agent pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/exos-agent/agents/` ili `.exos-agent/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, Exos Agent će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`exos-agent run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/exos-agent/commands/` ili `.exos-agent/commands/`. [Saznajte više ovdje](/docs/commands).
...
Exos Agent će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, Exos Agent **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju Exos Agent sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.exos-agent/plugins/` ili `~/.config/exos-agent/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite Exos Agent da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
