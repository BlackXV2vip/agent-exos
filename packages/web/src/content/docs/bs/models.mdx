...
Exos Agent koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM providera**, uključujući lokalne modele.

## Provajderi

Većina popularnih provajdera su unaprijed učitani prema zadanim postavkama. Ako ste dodali vjerodajnice za provajdera putem naredbe `/connect`, oni će biti dostupni kada pokrenete Exos Agent.
...
Evo nekoliko modela koji dobro rade sa Exos Agent, bez posebnog redosleda. (Ovo nije potpuna lista niti je nužno ažurirana):
...
Ovdje je puni ID `provider_id/model_id`. Na primjer, ako koristite [Exos Agent Zen](/docs/zen), koristili biste `exos-agent/gpt-5.1-codex` za GPT 5.1 Codex.
...
Mnogi modeli podržavaju više varijanti sa različitim konfiguracijama. Exos Agent se isporučuje sa ugrađenim podrazumevanim varijantama za popularne provajdere.

### Ugrađene varijante

Exos Agent se isporučuje sa zadanim varijantama za mnoge provajdere:
**Anthropic**:
...
Kada se Exos Agent pokrene, on provjerava modele u sljedećem prioritetnom redoslijedu:

1. Oznaka komandne linije `--model` ili `-m`. Format je isti kao u konfiguracijskoj datoteci: `provider_id/model_id`.
2. Lista modela u Exos Agent konfiguraciji.
