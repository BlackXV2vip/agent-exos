---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Exos Agent.
---

Dodaci vam omogućavaju da proširite Exos Agent spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Exos Agent.
...
- `.exos-agent/plugins/` - Dodaci na nivou projekta
- `~/.config/exos-agent/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/exos-agent/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/exos-agent/exos-agent.json`)
2. Konfiguracija projekta (`exos-agent.json`)
3. Globalni direktorij dodataka (`~/.config/exos-agent/plugins/`)
4. Direktorij dodataka projekta (`.exos-agent/plugins/`)
...
Exos Agent pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Exos Agent SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Exos Agent.
...
Spriječite exos-agent da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u exos-agent:
...
Pomoćnik `tool` kreira prilagođeni alat koji exos-agent može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za exos-agent zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://exos-agent.ai/docs/sdk) za detalje.
