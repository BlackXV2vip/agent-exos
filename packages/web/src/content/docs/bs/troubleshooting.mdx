...
Da biste otklonili probleme s Exos Agent, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\exos-agent\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `exos-agent --log-level DEBUG`.

---

## Pohrana

Exos Agent pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/exos-agent/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\exos-agent`
...
Exos Agent Desktop pokreće lokalni Exos Agent server (`exos-agent-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `ExosAgent` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/exos-agent/exos-agent.jsonc` (ili `~/.config/exos-agent/exos-agent.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/exos-agent/exos-agent.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\exos-agent\exos-agent.jsonc`
...
Exos Agent također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/exos-agent/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\exos-agent\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Exos Agent mogao ponovo izgraditi.

1. Potpuno zatvorite Exos Agent Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/exos-agent`
- **Linux**: obrišite `~/.cache/exos-agent` (ili pokrenite `rm -rf ~/.cache/exos-agent`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\exos-agent`

3. Ponovo pokrenite Exos Agent Desktop.

---

### Rješavanje problema sa vezom na serveru

Exos Agent Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `exos-agent.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `EXOS_AGENT_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `EXOS_AGENT_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, Exos Agent Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Exos Agent karakteristikama.

---

### Obavještenja se ne prikazuju

Exos Agent Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Exos Agent u postavkama vašeg OS-a, i
...
1. Zatvorite Exos Agent Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Exos Agent Desktop aplikacije):

- `exos-agent.settings.dat` (URL zadanog servera za desktop)
- `exos-agent.global.dat` i `exos-agent.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Ako imate problema s Exos Agent:
...
Da saznate kojim modelima imate pristup, pokrenite `exos-agent models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\exos-agent`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Exos Agent dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\exos-agent`

2. Ponovo pokrenite Exos Agent da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Exos Agent da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Exos Agent će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
