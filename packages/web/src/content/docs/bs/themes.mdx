...
U Exos Agent mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Exos Agent koristi nasu `exos-agent` temu.
...
Exos Agent dolazi sa vise ugradenih tema.
...
- Zele da Exos Agent odgovara izgledu njihovog terminala
...
Exos Agent podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/exos-agent/themes/*.json` ili `$XDG_CONFIG_HOME/exos-agent/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.exos-agent/themes/*.json`
4. **Trenutni radni direktorij** - `./.exos-agent/themes/*.json`
