...
Postoje dvije vrste agenata u Exos Agent: primarni agenti i podagenti.
...
Exos Agent dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
Exos Agent dolazi sa tri ugrađena subagenta, **General**, **Explore** i **Scout**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

Exos Agent dolazi sa dva ugrađena primarna agenta i tri ugrađena subagenta.
...
Agent samo za čitanje za istraživanje eksterne dokumentacije i zavisnosti. Koristite ga kada trebate klonirati repozitorij zavisnosti u Exos Agent-ov upravljani cache, pregledati izvorni kod biblioteke ili uporediti lokalni kod sa upstream implementacijama bez mijenjanja vašeg radnog prostora.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `exos-agent.json`:
...
- Globalno: `~/.config/exos-agent/agents/`
- Po projektu: `.exos-agent/agents/`
...
Ako temperatura nije navedena, Exos Agent koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu Exos Agent konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj Exos Agent konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [Exos Agent Zen](/docs/zen), koristili biste `exos-agent/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `exos-agent models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/BlackXV2vip/agent-exos).
:::
