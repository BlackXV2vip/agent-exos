...
Možete dodati vanjske alate u Exos Agent koristeći _Model Context Protocol_, ili MCP. Exos Agent podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Exos Agent Config](https://exos-agent.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/exos-agent`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
Exos Agent automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Exos Agent će:
...
Ako server zahtijeva autentifikaciju, Exos Agent će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `exos-agent mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Exos Agent sigurno čuva tokene u `~/.local/share/exos-agent/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Exos Agent, zajedno s ugrađenim alatima. Možete njima upravljati kroz Exos Agent konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Exos Agent sa vašim Sentry nalogom.
