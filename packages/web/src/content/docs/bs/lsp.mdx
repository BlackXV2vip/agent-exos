---
title: LSP serveri
description: Exos Agent se integriše sa vašim LSP serverima.
---

Exos Agent se može integrisati sa Language Server Protocol (LSP) serverima kako bi koristio dijagnostiku kao feedback za agenta.

## Ugrađeni

Exos Agent dolazi sa nekoliko ugrađenih LSP servera za popularne jezike:
...
:::note
Možete onemogućiti automatska preuzimanja LSP servera tako što ćete postaviti varijablu okruženja `EXOS_AGENT_DISABLE_LSP_DOWNLOAD` na `true`.
:::
...
Kada je LSP omogućen i exos-agent otvori fajl, on:
...
Možete omogućiti i prilagoditi LSP servere kroz `lsp` odjeljak u vašoj exos-agent konfiguraciji.
