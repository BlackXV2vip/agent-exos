---
title: Server
description: Komunicirajte s exos-agent serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `exos-agent serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji exos-agent klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `exos-agent.local` |
...
Postavite `EXOS_AGENT_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `exos-agent`, ili postavite `EXOS_AGENT_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `exos-agent serve` i za `exos-agent web`.
...
Kada pokrenete `exos-agent`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite exos-agent server za programsku interakciju sa exos-agent-om.
:::

Ova arhitektura omogucava exos-agent podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `exos-agent serve` da startate standalone server. Ako je exos-agent TUI vec pokrenut, `exos-agent serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Exos Agent [IDE](/docs/ide) pluginovi.
...
exos-agent server izlaže sljedece API-je.
