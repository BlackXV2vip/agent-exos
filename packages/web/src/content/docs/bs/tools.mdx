...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. Exos Agent dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u Exos Agent.
...
:::note
Ovaj alat je dostupan samo kada je `EXOS_AGENT_EXPERIMENTAL_LSP_TOOL=true` (ili `EXOS_AGENT_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz Exos Agent provajdera ili kada je varijabla `EXOS_AGENT_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju Exos Agent:
