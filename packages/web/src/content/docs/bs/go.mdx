...
Exos Agent Go je povoljna pretplata — **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno** — koja vam pruža pouzdan pristup popularnim otvorenim modelima za programiranje.

Go radi kao bilo koji drugi provajder u Exos Agent-u. Pretplatite se na Exos Agent Go i
dobijete svoj API ključ. On je **potpuno opcionalan** i ne morate ga koristiti da
biste koristili Exos Agent.
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa Exos Agent-om.
:::
...
Exos Agent Go vam daje pristup ovim modelima za **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno**.

---

## Kako funkcioniše

Exos Agent Go radi kao bilo koji drugi provajder u Exos Agent-u.

1. Prijavite se na **<a href={console}>Exos Agent Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite komandu `/connect` u TUI-ju, odaberite `ExosAgent Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih kroz Go.

:::note
Samo jedan član po radnom prostoru (workspace) može se pretplatiti na Exos Agent Go.
:::
...
Exos Agent Go uključuje sljedeća ograničenja:
...
[Model id](/docs/config/#models) u vašoj Exos Agent konfiguraciji
koristi format `exos-agent-go/<model-id>`. Na primjer, za Kimi K3, koristili biste
`exos-agent-go/kimi-k3` u svojoj konfiguraciji.
...
Napravili smo Exos Agent Go da bismo:
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz Exos Agent koristite i bilo kojeg drugog provajdera.
