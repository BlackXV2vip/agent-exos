...
Agent skills omogucavaju Exos Agent da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
Exos Agent pretrazuje ove lokacije:

- Konfiguracija projekta: `.exos-agent/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/exos-agent/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, Exos Agent ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.exos-agent/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/exos-agent/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.exos-agent/skills/git-release/SKILL.md` ovako:
...
Exos Agent navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `exos-agent.json`:
...
**Za ugradene agente** (u `exos-agent.json`):
