...
With Exos Agent you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Exos Agent uses our own `exos-agent` theme.
...
Exos Agent comes with several built-in themes.
...
- Want Exos Agent to match their terminal's appearance
...
Exos Agent supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/exos-agent/themes/*.json` or `$XDG_CONFIG_HOME/exos-agent/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.exos-agent/themes/*.json`
4. **Current working directory** - Defined in `./.exos-agent/themes/*.json`
