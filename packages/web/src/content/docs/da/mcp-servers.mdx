...
Du kan tilføje eksterne værktøjer til Exos Agent ved hjælp af _Model Context Protocol_ eller MCP. Exos Agent understøtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i din [Exos Agent Config](https://exos-agent.ai/docs/config/) under `mcp`. Tilføj hver MCP med et unikt navn. Du kan henvise til denne MCP ved navn, når du spørger LLM.
...
Organisationer kan levere standard MCP-servere via deres `.well-known/exos-agent`-slutpunkt. Disse servere kan være deaktiveret som standard, hvilket giver brugerne mulighed for at tilmelde sig dem, de har brug for.
...
Exos Agent håndterer automatisk OAuth-godkendelse for eksterne MCP-servere. Når en server kræver godkendelse, vil Exos Agent:
...
Hvis serveren kræver godkendelse, vil Exos Agent bede dig om at godkende, når du prøver at bruge den første gang. Hvis ikke, kan du [manually trigger the flow](#authenticating) med `exos-agent mcp auth <server-name>`.
...
Kommandoen `mcp auth` åbner din browser for godkendelse. Når du har godkendt, vil Exos Agent opbevare tokens sikkert i `~/.local/share/exos-agent/mcp-auth.json`.
...
Dine MCPs er tilgængelige som værktøjer i **TK\_** sammen med indbyggede værktøjer. Så du kan administrere dem gennem Exos Agent-konfigurationen som ethvert andet værktøj.
...
Dette åbner et browservindue for at fuldføre OAuth-flowet og forbinde Exos Agent til din Sentry-konto.
