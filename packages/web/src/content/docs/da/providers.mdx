---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i Exos Agent.
---

import config from "../../../../config.mjs"
export const console = config.console

Exos Agent bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i Exos Agent-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/exos-agent/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i Exos Agent-konfigurationen.
...
Exos Agent Zen er en liste over modeller leveret af Exos Agent-teamet, som er blevet testet og verificeret til at fungere godt med Exos Agent. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med Exos Agent Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg exos-agent og gå til [exos-agent.ai/auth](https://exos-agent.ai/auth).
...
Det fungerer som alle andre udbydere i Exos Agent og er helt valgfrit at bruge.

---

## Exos Agent Go

Exos Agent Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af Exos Agent-teamet, som er testet og verificeret til at fungere godt med Exos Agent.

1. Kør kommandoen `/connect` i TUI, vælg `ExosAgent Go`, og gå til [exos-agent.ai/auth](https://exos-agent.ai/zen).
...
Sådan bruger du Amazon Bedrock med Exos Agent:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører exos-agent:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `exos-agent.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i Exos Agent understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere exos-agent til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at exos-agent skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem exos-agent-konfigurationen.
...
gemme tokenet i exos-agent auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
Exos Agent bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse Exos Agent
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`exos-agent.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [exos-agent-gitlab-auth](https://www.npmjs.com/package/exos-agent-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `exos-agent.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med Exos Agent:
...
Sådan bruger du Google Vertex AI med Exos Agent:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører exos-agent.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem exos-agent, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [exos-agent-helicone-session](https://github.com/H2Shami/exos-agent-helicone-session) plugin for automatisk at logge hver Exos Agent-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver Exos Agent-samtale opført som en separat session.
...
Du kan konfigurere exos-agent til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere exos-agent til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere exos-agent til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for Exos Agent. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/exos-agent) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med Exos Agent:
...
3. Kopier API-nøglen til brug i Exos Agent.
...
6. **Vigtigt**: Før du bruger skymodeller i Exos Agent, skal du hente modelinformationen lokalt:
...
Exos Agent Zen er en liste over testede og verificerede modeller leveret af Exos Agent-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>Exos Agent Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **Exos Agent Zen**.

   ```txt
   /connect
   ```

3. Indtast Exos Agent API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem exos-agent-konfigurationen.
...
5. Du kan også tilpasse dem gennem exos-agent-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Exos Agent:
...
Du kan også tilpasse modeller gennem exos-agent-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med exos-agent. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `exos-agent.json`-filen i projektkataloget:
...
`limit`-felterne lader Exos Agent forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `exos-agent auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek exos-agent-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i exos-agent-konfigurationen.
