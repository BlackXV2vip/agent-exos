---
title: Plugins
description: Skriv dine egne plugins for at udvide Exos Agent.
---

Plugins giver dig mulighed for at udvide Exos Agent ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Exos Agents standardadfærd.
...
- `.exos-agent/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/exos-agent/node_modules/`.
...
1. Global konfiguration (`~/.config/exos-agent/exos-agent.json`)
2. Projektkonfiguration (`exos-agent.json`)
3. Global plugin-mappe (`~/.config/exos-agent/plugins/`)
4. Projekt plugin bibliotek (`.exos-agent/plugins/`)
...
Exos Agent kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En exos-agent SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide exos-agent.
...
:::note
Hvis du bruger Exos Agent desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå exos-agent i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til exos-agent:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som exos-agent kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for exos-agent sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://exos-agent.ai/docs/sdk) for detaljer.
