...
Exos Agent Go er et lavprisabonnement — **$5 for din første måned**, derefter **$10/måned** — der giver dig pålidelig adgang til populære åbne kodningsmodeller.

Go fungerer som enhver anden udbyder i Exos Agent. Du abonnerer på Exos Agent Go og
...
:::tip
Vi testede en udvalgt gruppe af modeller og udbydere, der fungerer godt med Exos Agent.
:::
...
Exos Agent Go giver dig adgang til disse modeller for **$5 for din første måned**, derefter **$10/måned**.

---

## Sådan fungerer det

Exos Agent Go fungerer som enhver anden udbyder i Exos Agent.

1. Du logger ind på **<a href={console}>Exos Agent Zen</a>**, abonnerer på Go, og
   kopierer din API-nøgle.
2. Du kører kommandoen `/connect` i TUI'en, vælger `ExosAgent Go`, og indsætter
   din API-nøgle.
3. Kør `/models` i TUI'en for at se listen over tilgængelige modeller gennem Go.

:::note
Kun ét medlem per arbejdsområde kan abonnere på Exos Agent Go.
:::
...
Exos Agent Go inkluderer følgende grænser:
...
Dit [model id](/docs/config/#models) i din Exos Agent config
bruger formatet `exos-agent-go/<model-id>`. For eksempel for Kimi K3, vil du
bruge `exos-agent-go/kimi-k3` i din config.
...
Vi skabte Exos Agent Go for at:
...
4. Undgå **lock-in** ved også at lade dig bruge enhver anden udbyder med Exos Agent.
