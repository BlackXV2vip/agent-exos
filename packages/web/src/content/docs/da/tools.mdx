...
Her er alle de innebygde verktøyene som er tilgængelige i Exos Agent.
...
:::note
Dette verktøyet er bare tilgængelig når `EXOS_AGENT_EXPERIMENTAL_LSP_TOOL=true` (eller `EXOS_AGENT_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger Exos Agent-leverandøren eller når miljøvariabelen `EXOS_AGENT_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
