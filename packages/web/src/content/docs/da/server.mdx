---
title: Server
description: Interager med exos-agent-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `exos-agent serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en exos-agent-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `exos-agent.local` |
...
Indstil `EXOS_AGENT_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `exos-agent`, eller indstil `EXOS_AGENT_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `exos-agent serve` og `exos-agent web`.
...
Når du kører `exos-agent` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug exos-agent-serveren til at interagere med exos-agent programmatisk.
:::

Denne arkitektur lader exos-agent understøtte flere klienter og lader dig interagere med exos-agent programmatisk.

Du kan køre `exos-agent serve` for at starte en selvstændig server. Hvis du har
exos-agent TUI kørende, vil `exos-agent serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af Exos Agent [IDE](/docs/ide) plugins.
...
Exos Agent-serveren viser følgende API'er.
