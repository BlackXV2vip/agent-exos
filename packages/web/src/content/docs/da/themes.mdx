...
Med Exos Agent kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger Exos Agent vårt eget `exos-agent`-tema.
...
Exos Agent støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/exos-agent/themes/*.json` eller `$XDG_CONFIG_HOME/exos-agent/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.exos-agent/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.exos-agent/themes/*.json`
