---
title: TUI
description: Brug af Exos Agent-terminalbrugergrænsefladen.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Exos Agent giver en interaktiv terminalgrænseflade eller TUI til at arbejde med dine projekter med en LLM.

At køre Exos Agent starter TUI for den nuværende mappe.
...
Når du bruger Exos Agent TUI, kan du skrive `/` efterfulgt af et kommandonavn for hurtigt at udføre handlinger. For eksempel:
...
Tilføj en udbyder til Exos Agent. Lader dig vælge fra tilgængelige udbydere og tilføje deres API-nøgler.
...
Afslut Exos Agent. _Aliaser_: `/quit`, `/q`
...
Dette er adskilt fra `exos-agent.json`, som konfigurerer server-/kørselstidsadfærd.
...
- `leader_timeout` - Styrer hvor længe Exos Agent venter efter leader key. Standard er `2000`.
...
- `sound_pack` - Sound pack-ID der skal bruges. Standard er `exos-agent.default`.
