...
Exos Agent uses the `permission` config to decide whether a given action should run automatically, prompt you, or be blocked.
...
Start Exos Agent with `--auto` to automatically approve permission requests that are not explicitly denied.

```bash
exos-agent --auto
```

You can also use auto mode with [`exos-agent run`](/docs/cli#run).

```bash
exos-agent run --auto "Refactor this module"
```
...
Use `external_directory` to allow tool calls that touch paths outside the working directory where Exos Agent was started. This applies to any tool that takes a path as input (for example `read`, `edit`, `glob`, `grep`, and many `bash` commands).
...
Exos Agent permissions are keyed by tool name, plus a couple of safety guards:
...
If you don’t specify anything, Exos Agent starts from permissive defaults:
...
When Exos Agent prompts for approval, the UI offers three outcomes:

- `once` — approve just this request
- `always` — approve future requests matching the suggested patterns (for the rest of the current Exos Agent session)
