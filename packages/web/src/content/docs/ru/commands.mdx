...
Создайте `.exos-agent/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию exos-agent или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем exos-agent [config](/docs/config):
...
- Глобальный: `~/.config/exos-agent/commands/`
- Для каждого проекта: `.exos-agent/commands/`
...
exos-agent включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
