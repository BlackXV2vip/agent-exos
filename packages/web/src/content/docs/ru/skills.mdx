...
Навыки агента позволяют exos-agent обнаруживать многократно используемые инструкции из вашего репозитория или домашнего каталога.
...
exos-agent выполняет поиск в следующих местах:

- Конфигурация проекта: `.exos-agent/skills/<name>/SKILL.md`
- Глобальная конфигурация: `~/.config/exos-agent/skills/<name>/SKILL.md`.
...
Для локальных путей проекта exos-agent переходит из вашего текущего рабочего каталога, пока не достигнет рабочего дерева git.
Он загружает все соответствующие `skills/*/SKILL.md` в `.exos-agent/` и все соответствующие `.claude/skills/*/SKILL.md` или `.agents/skills/*/SKILL.md` по пути.

Глобальные определения также загружаются из `~/.config/exos-agent/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` и `~/.agents/skills/*/SKILL.md`.
...
Создайте `.exos-agent/skills/git-release/SKILL.md` следующим образом:
...
exos-agent перечисляет доступные навыки в описании инструмента `skill`.
...
Контролируйте, к каким навыкам агенты могут получить доступ, используя разрешения на основе шаблонов в `exos-agent.json`:
...
**Для встроенных агентов** (в формате `exos-agent.json`):
