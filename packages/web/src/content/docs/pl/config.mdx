---
title: Konfiguracja
description: Ustawienia Exos Agent w pliku JSON.
---

Możesz dostosować Exos Agent za pomocą pliku konfiguracyjnego JSON.

---

## Format

Exos Agent obsługuje formaty **JSON** i **JSONC** (JSON z komentarzami).
...
Na przykład, jeśli konfiguracja globalna ustawi `theme: "exos-agent"` i `autoupdate: true`, a konfiguracja projektu ustawi `model: "anthropic/claude-sonnet-4-5"`, ostateczna konfiguracja będzie zawierać wszystkie trzy ustawienia.
...
1. **Konfiguracja zdalna** (z `.well-known/exos-agent`) – ustawienia wymuszane przez organizację
2. **Konfiguracja globalna** (`~/.config/exos-agent/exos-agent.json`) – preferencje użytkownika
3. **Konfiguracja niestandardowa** (zmienna `EXOS_AGENT_CONFIG`) — jawne nadpisanie
4. **Konfiguracja projektu** (`exos-agent.json` w projekcie) - ustawienia specyficzne dla projektu
5. **Katalogi `.exos-agent`** - agenci, polecenia, umiejętności
6. **Konfiguracja wbudowana** (zmienna `EXOS_AGENT_CONFIG_CONTENT`) — nadpisanie środowiska uruchomieniowego
...
:::note
W katalogach `.exos-agent` i `~/.config/exos-agent` używane są **nazwy w liczbie mnogiej** dla podkatalogów: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Nazwy w liczbie pojedynczej (np. `agent/`) są również obsługiwane dla kompatybilności wstecznej.
:::
...
Organizacje mogą publikować konfigurację pod adresem `.well-known/exos-agent`. Jest ona pobierana automatycznie po uwierzytelnieniu u dostawcy, który ją obsługuje.
...
Umieść swoją globalną konfigurację Exos Agent w `~/.config/exos-agent/exos-agent.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak dostawcy, modele i uprawnienia.

Dla ustawień specyficznych dla TUI, użyj `~/.config/exos-agent/tui.json`.
...
Dodaj `exos-agent.json` w katalogu głównym projektu. Konfiguracja projektu ma najwyższy priorytet wśród plików konfiguracyjnych — nadpisuje konfiguracje globalne i zdalne.
...
Kiedy Exos Agent się uruchamia, szuka pliku konfiguracyjnego w katalogu głównym repozytorium Git.
...
Możesz załadować niestandardowy plik konfiguracyjny, ustawiając zmienną środowiskową `EXOS_AGENT_CONFIG`.
...
Możesz ustawić niestandardowy katalog konfiguracyjny za pomocą zmiennej środowiskowej `EXOS_AGENT_CONFIG_DIR`. Katalog ten będzie przeszukiwany pod kątem agentów, poleceń, trybów i wtyczek, tak jak standardowy katalog `.exos-agent` i powinien zachować tę samą strukturę.
...
Katalog niestandardowy jest ładowany po globalnej konfiguracji i katalogach `.exos-agent`, więc **może być łączony** z ich zawartością.

---

## Schemat

Plik konfiguracyjny ma schemat JSON dostępny pod adresem [**`exos-agent.ai/config.json`**](https://exos-agent.ai/config.json).
...
Użyj `EXOS_AGENT_TUI_CONFIG`, aby wskazać niestandardowy plik konfiguracyjny TUI.

Przestarzałe klucze `theme`, `keybinds` i `tui` w `exos-agent.json` są wycofywane i automatycznie migrowane, gdy to możliwe.
...
Możesz skonfigurować serwer dla `exos-agent serve` i `exos-agent web` za pomocą opcji `server`.
...
- `mdns` - Włącz wykrywanie usług mDNS. Dzięki temu inne urządzenia w sieci mogą łatwo znaleźć Twój serwer Exos Agent.
- `mdnsDomain` - Niestandardowa nazwa domeny dla usług mDNS. Domyślnie `exos-agent.local`. Przydatne do uruchamiania wielu instancji w tej samej sieci.
...
Opcja `small_model` konfiguruje oddzielny model do lżejszych zadań, takich jak generowanie tytułów. Domyślnie Exos Agent wybiera tańszy model od tego samego dostawcy, jeśli jest dostępny, w przeciwnym razie powraca do modelu głównego.
...
Możesz także definiować agentów przy użyciu plików markdown w `~/.config/exos-agent/agents/` lub `.exos-agent/agents/`. [Dowiedz się więcej o agentach](/docs/agents).
...
Agent domyślny musi być agentem typu `primary` (nie `subagent`). Może to być agent wbudowany, taki jak `"build"` lub `"plan"`, albo zdefiniowany przez Ciebie [agent niestandardowy](/docs/agents). Jeśli agent nie istnieje lub jest subagentem, Exos Agent powróci do `"build"` z ostrzeżeniem.

Dotyczy to wszystkich interfejsów: TUI, CLI (`exos-agent run`), aplikacji desktopowej i GitHub Actions.
...
Możesz także definiować polecenia przy użyciu plików Markdown w `~/.config/exos-agent/commands/` lub `.exos-agent/commands/`. [Dowiedz się więcej o poleceniach](/docs/commands).
...
Exos Agent może automatycznie pobierać nowe wersje. Możesz to kontrolować za pomocą opcji `autoupdate`.
...
Działa to tylko wtedy, gdy Exos Agent nie został zainstalowany przez menedżera pakietów, takiego jak Homebrew.
...
Domyślnie Exos Agent **zezwala na wszystkie działania** bez konieczności zatwierdzania. Możesz to zmienić za pomocą opcji `permission`.
...
[Wtyczki](/docs/plugins) rozszerzają Exos Agent o niestandardowe narzędzia, hooki i integracje.

Umieść pliki wtyczek w `.exos-agent/plugins/` lub `~/.config/exos-agent/plugins/`. Możesz także załadować wtyczkę z npm za pomocą opcji `plugin`.
...
Jest to przydatne, gdy chcesz ograniczyć użycie Exos Agent tylko do określonych dostawców.
