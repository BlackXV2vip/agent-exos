...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. Exos Agent posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w Exos Agent.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `EXOS_AGENT_EXPERIMENTAL_LSP_TOOL=true` (lub `EXOS_AGENT_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy Exos Agent lub gdy zmienna środowiskowa `EXOS_AGENT_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu Exos Agent:
