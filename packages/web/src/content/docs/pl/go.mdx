...
Exos Agent Go to niskokosztowa subskrypcja — **5 $ za pierwszy miesiąc**, a następnie **10 $/miesiąc** — która zapewnia niezawodny dostęp do popularnych otwartych modeli do kodowania.

Go działa jak każdy inny dostawca w Exos Agent. Subskrybujesz Exos Agent Go i
...
Exos Agent Go daje Ci dostęp do tych modeli za **5 $ za pierwszy miesiąc**, a następnie **10 $/miesiąc**.

---

## Jak to działa

Exos Agent Go działa jak każdy inny dostawca w Exos Agent.

1. Logujesz się do **<a href={console}>Exos Agent Zen</a>**, subskrybujesz Go i
   kopiujesz swój klucz API.
2. Uruchamiasz komendę `/connect` w TUI, wybierasz `ExosAgent Go` i wklejasz
   swój klucz API.
3. Uruchom `/models` w TUI, aby zobaczyć listę modeli dostępnych w ramach Go.

:::note
Tylko jeden członek na obszar roboczy (workspace) może zasubskrybować Exos Agent Go.
:::
...
Exos Agent Go zawiera następujące limity:
...
[ID modelu](/docs/config/#models) w Twojej konfiguracji Exos Agent
używa formatu `exos-agent-go/<model-id>`. Na przykład dla Kimi K3 należy użyć
`exos-agent-go/kimi-k3` w swojej konfiguracji.
...
Stworzyliśmy Exos Agent Go, aby:
...
4. Zapewnić **brak ograniczeń (no lock-in)**, pozwalając na korzystanie z dowolnego innego dostawcy wraz z Exos Agent.
