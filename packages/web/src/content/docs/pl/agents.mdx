...
W Exos Agent obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
Exos Agent zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
Exos Agent ma trzech wbudowanych subagentów: **General**, **Explore** i **Scout**. Przyjrzymy się im poniżej.

---

## Wbudowane

Exos Agent ma dwa wbudowane agenty główne i trzech wbudowanych subagentów.
...
Agent tylko do odczytu do pracy z zewnętrzną dokumentacją i badaniem zależności. Używaj go, gdy chcesz sklonować repozytorium zależności do zarządzanej pamięci podręcznej Exos Agent, przejrzeć kod źródłowy biblioteki albo porównać lokalny kod z implementacjami upstream bez modyfikowania swojego workspace.
...
Skonfiguruj agenta w pliku `exos-agent.json`:
...
- Globalnie: `~/.config/exos-agent/agents/`
- W projekcie: `.exos-agent/agents/`
...
Jeśli nie zostanie skonfigurowana, Exos Agent używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji Exos Agent, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji Exos Agent w `provider/model-id`. Na przykład, [Exos Agent Zen](/docs/zen), użyj `exos-agent/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `exos-agent models`, aby zobaczyć dostępne modele.
:::
...
:::tip
Masz agenta, którym warto się podzielić? [Prześlij PR](https://github.com/BlackXV2vip/agent-exos).
:::
