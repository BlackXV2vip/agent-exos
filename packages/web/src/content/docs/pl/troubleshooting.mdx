...
Aby debugować problemy z exos-agent, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\exos-agent\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `exos-agent --log-level DEBUG`.

---

## Przechowywanie danych

exos-agent przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/exos-agent/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\exos-agent`
...
exos-agent Desktop uruchamia w tle lokalny serwer exos-agent (sidecar `exos-agent-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `ExosAgent` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/exos-agent/exos-agent.jsonc` (lub `~/.config/exos-agent/exos-agent.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/exos-agent/exos-agent.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\exos-agent\exos-agent.jsonc`
...
exos-agent może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/exos-agent/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\exos-agent\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby exos-agent mógł ją odbudować.

1. Całkowicie zamknij exos-agent Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/exos-agent`
- **Linux**: usuń `~/.cache/exos-agent` (lub uruchom `rm -rf ~/.cache/exos-agent`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\exos-agent`

3. Uruchom ponownie exos-agent Desktop.

---

### Napraw problemy z połączeniem z serwerem

exos-agent Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `exos-agent.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `EXOS_AGENT_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `EXOS_AGENT_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows exos-agent Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami exos-agent.

---

### Brak powiadomień

exos-agent Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla exos-agent w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie exos-agent Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji exos-agent Desktop):

- `exos-agent.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `exos-agent.global.dat` i `exos-agent.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
Jeśli masz problemy z exos-agent:
...
3. Upewnij się, że masz najnowszą wersję z `exos-agent upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `exos-agent models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\exos-agent`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. exos-agent dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\exos-agent`

2. Uruchom ponownie kod exos-agent, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to exos-agent do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
exos-agent wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
