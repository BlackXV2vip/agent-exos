...
Dzięki exos-agent możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie exos-agent używa naszego własnego motywu `exos-agent`.
...
exos-agent ma kilka wbudowanych motywów.
...
- Chcą, aby Exos Agent pasował do wyglądu ich terminala
...
exos-agent obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/exos-agent/themes/*.json` lub `$XDG_CONFIG_HOME/exos-agent/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.exos-agent/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.exos-agent/themes/*.json`
