---
title: TUI
description: Korzystanie z interfejsu użytkownika terminala exos-agent.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

exos-agent zapewnia interaktywny interfejs terminala lub TUI do pracy nad projektami za pomocą LLM.

Uruchomienie exos-agent uruchamia TUI dla bieżącego katalogu.
...
Korzystając z TUI exos-agent, możesz wpisać `/`, po którym następuje nazwa polecenia, aby szybko wykonać działania. Na przykład:
...
Dodaj dostawcę do exos-agent. Umożliwia wybór dostępnych dostawców i dodanie ich kluczy API.
...
Zamknij exos-agent. _Aliasy_: `/quit`, `/q`
...
Jest to oddzielny plik od `exos-agent.json`, który konfiguruje zachowanie serwera/runtime.
...
- `leader_timeout` - Określa, jak długo Exos Agent czeka po naciśnięciu leader key. Domyślnie `2000`.
...
- `sound_pack` - ID sound pack do użycia. Domyślnie `exos-agent.default`.
