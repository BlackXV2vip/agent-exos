...
Utwórz `.exos-agent/commands/test.md`:
...
Możesz zdefiniować polecenia w `exos-agent.json` lub w plikach w katalogu `commands/`.

---

### JSON

Użyj opcji `command` w swojej konfiguracji exos-agent:
...
- Globalnie: `~/.config/exos-agent/commands/`
- Na projekt: `.exos-agent/commands/`
...
exos-agent zawiera kilka wbudowanych poleceń, takich jak `/init`, `/undo`, `/redo`, `/share`, `/help`; [dowiedz się więcej](/docs/tui#commands).
