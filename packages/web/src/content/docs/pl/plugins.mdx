---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć exos-agent.
---

Wtyczki umożliwiające wyłączenie exos-agent poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania exos-agent.
...
- `.exos-agent/plugins/` - Wtyczki na poziomie projektu
- `~/.config/exos-agent/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/exos-agent/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/exos-agent/exos-agent.json`)
2. Konfiguracja projektu (`exos-agent.json`)
3. Globalny katalog wtyczek (`~/.config/exos-agent/plugins/`)
4. Katalog wtyczek projektu (`.exos-agent/plugins/`)
...
exos-agent uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z exos-agent do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do exos-agent.
...
:::note
Jeśli korzystasz z aplikacji komputerowej exos-agent, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj exos-agent czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do exos-agent:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod exos-agent. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://exos-agent.ai/docs/sdk).
