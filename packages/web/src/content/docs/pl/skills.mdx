...
Umiejętności agenta pozwalają exos-agent odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
exos-agent przeszukuje te lokalizacje:

- Project config: `.exos-agent/skills/<name>/SKILL.md`
- Global config: `~/.config/exos-agent/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu exos-agent przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.exos-agent/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/exos-agent/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.exos-agent/skills/git-release/SKILL.md` w ten sposób:
...
exos-agent wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `exos-agent.json`:
...
**Dla agentów wbudowanych** (w `exos-agent.json`):
