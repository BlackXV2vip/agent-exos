...
Możesz dodać dodatkowe informacje do exos-agent za pomocą _Model Context Protocol_, w skrócie MCP. exos-agent obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [exos-agent Config](https://exos-agent.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/exos-agent`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
exos-agent automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, exos-agent:
...
Jeśli serwer wymaga uwierzytelnienia, exos-agent poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `exos-agent mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji exos-agent bezpieczne przechowa tokeny w `~/.local/share/exos-agent/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w exos-agent, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie exos-agent, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć exos-agent z kontem Sentry.
