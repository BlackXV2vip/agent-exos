---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w exos-agent.
---

import config from "../../../../config.mjs"
export const console = config.console

exos-agent używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji exos-agent.
...
w `~/.local/share/exos-agent/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim exos-agent
config.
...
Exos Agent Zen to lista modeli dostarczonych przez zespół exos-agent, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z exos-agent. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Exos Agent Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `ExosAgent Zen` i przejdź do [exos-agent.ai/auth](https://exos-agent.ai/zen).
...
Działa jak każdy inny dostawca w exos-agent i jest całkowicie opcjonalny w użyciu.

---

## Exos Agent Go

Exos Agent Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół exos-agent, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z exos-agent.

1. Uruchom polecenie `/connect` w TUI, wybierz `ExosAgent Go` i przejdź do [exos-agent.ai/auth](https://exos-agent.ai/zen).
...
Aby używać Amazon Bedrock z exos-agent:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania exos-agent:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `exos-agent.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w exos-agent nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować exos-agent do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod exos-agent działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji exos-agent.
...
exos-agent używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować exos-agent
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`exos-agent.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [exos-agent-gitlab-auth](https://www.npmjs.com/package/exos-agent-gitlab-auth).
...
Aby korzystać z subskrypcji GitHub Copilot z kodem exos-agent:
...
Aby używać Google Vertex AI z exos-agent:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania exos-agent.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą exos-agent, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [exos-agent-helicone-session](https://github.com/H2Shami/exos-agent-helicone-session), aby automatycznie rejestrować każdą konwersację exos-agent jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja exos-agent będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować exos-agent tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować exos-agent tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować exos-agent tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem exos-agent. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/exos-agent).
:::
...
Aby korzystać z Ollama Cloud z exos-agent:
...
3. Skopiuj klucz API do użycia w exos-agent.
...
6. **Ważne**: Przed użyciem modeli chmurowych w exos-agent musisz pobrać informacje o modelu lokalnie:
...
Exos Agent Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół exos-agent. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Exos Agent Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Exos Agent Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API exos-agent.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji exos-agent.
...
5. Możesz je także dostosować za pomocą konfiguracji exos-agent. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z exos-agent:
...
Możesz także dostosować modele za pomocą konfiguracji exos-agent. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z exos-agent. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `exos-agent.json` w katalogu projektu:
...
Pola `limit` pozwalają exos-agent zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `exos-agent auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację exos-agent i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji exos-agent.
