---
title: Serwer
description: Interakcja z serwerem exos-agent za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `exos-agent serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient exos-agent.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `exos-agent.local` |
...
Ustaw `EXOS_AGENT_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `exos-agent` lub ustaw `EXOS_AGENT_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `exos-agent serve`, jak i `exos-agent web`.
...
Kiedy uruchomisz `exos-agent`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera exos-agent do programowej interakcji z exos-agent.
:::

Ta architektura umożliwia obsługę wielu klientów przez exos-agent i programową interakcję z kodem otwartym.

Możesz uruchomić `exos-agent serve`, aby uruchomić samodzielny serwer. Jeśli masz
exos-agent TUI działa, `exos-agent serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki exos-agent [IDE](/docs/ide).
...
Serwer exos-agent udostępnia następujące interfejsy API.
