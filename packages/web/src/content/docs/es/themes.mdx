...
Con Exos Agent puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, Exos Agent usa nuestro propio tema `exos-agent`.
...
Exos Agent viene con varios temas integrados.
...
- Quiere que Exos Agent coincida con la apariencia de su terminal
...
Exos Agent admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/exos-agent/themes/*.json` o `$XDG_CONFIG_HOME/exos-agent/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.exos-agent/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.exos-agent/themes/*.json`
