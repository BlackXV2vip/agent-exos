...
Las habilidades del agente permiten a Exos Agent descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Exos Agent busca estas ubicaciones:

- Configuración del proyecto: `.exos-agent/skills/<name>/SKILL.md`
- Configuración global: `~/.config/exos-agent/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, Exos Agent sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.exos-agent/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/exos-agent/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.exos-agent/skills/git-release/SKILL.md` así:
...
Exos Agent enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `exos-agent.json`:
...
**Para agentes integrados** (en `exos-agent.json`):
