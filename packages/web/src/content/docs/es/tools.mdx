...
Las herramientas permiten que LLM realice acciones en su código base. Exos Agent viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en Exos Agent.
...
:::note
Esta herramienta solo está disponible cuando `EXOS_AGENT_EXPERIMENTAL_LSP_TOOL=true` (o `EXOS_AGENT_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor Exos Agent o cuando la variable de entorno `EXOS_AGENT_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar Exos Agent:

```bash
EXOS_AGENT_ENABLE_EXA=1 exos-agent
```
