...
Para depurar problemas con Exos Agent, comience verificando los registros y los datos locales que almacena en el disco.
...
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\exos-agent\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `exos-agent --log-level DEBUG`.

---

## Almacenamiento

exos-agent almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/exos-agent/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\exos-agent`
...
Exos Agent Desktop ejecuta un servidor Exos Agent local (el sidecar `exos-agent-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `ExosAgent` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/exos-agent/exos-agent.jsonc` (o `~/.config/exos-agent/exos-agent.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/exos-agent/exos-agent.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\exos-agent\exos-agent.jsonc`
...
Exos Agent también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/exos-agent/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\exos-agent\plugins`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que Exos Agent pueda reconstruirlo.

1. Salga de Exos Agent Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/exos-agent`
- **Linux**: eliminar `~/.cache/exos-agent` (o ejecutar `rm -rf ~/.cache/exos-agent`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\exos-agent`

3. Reinicie el escritorio Exos Agent.

---

### Solucionar problemas de conexión del servidor

Exos Agent Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `exos-agent.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `EXOS_AGENT_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `EXOS_AGENT_PORT` (o elija un puerto libre) y reinicie.
...
En Windows, el escritorio Exos Agent requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de Exos Agent.

---

### Notificaciones que no se muestran

Exos Agent Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para Exos Agent en la configuración de su sistema operativo, y
...
1. Salga del escritorio Exos Agent.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio Exos Agent):

- `exos-agent.settings.dat` (URL del servidor predeterminado de escritorio)
- `exos-agent.global.dat` y `exos-agent.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
Si tiene problemas con Exos Agent:
...
3. Asegúrese de tener la última versión con `exos-agent upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `exos-agent models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\exos-agent`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. exos-agent instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\exos-agent`

2. Reinicie exos-agent para reinstalar los paquetes del proveedor más recientes.

Esto obligará a exos-agent a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
exos-agent detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
