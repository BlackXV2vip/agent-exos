---
title: Complementos
description: Escriba sus propios complementos para extender Exos Agent.
---

Los complementos le permiten extender Exos Agent al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Exos Agent.
...
- `.exos-agent/plugins/` - Complementos a nivel de proyecto
- `~/.config/exos-agent/plugins/` - Complementos globales
...
```json title="exos-agent.json"
{
  ...
  "plugin": ["exos-agent-helicone-session", "exos-agent-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/exos-agent/node_modules/`.
...
1. Configuración global (`~/.config/exos-agent/exos-agent.json`)
2. Configuración del proyecto (`exos-agent.json`)
3. Directorio global de complementos (`~/.config/exos-agent/plugins/`)
4. Directorio de complementos del proyecto (`.exos-agent/plugins/`)
...
```json title=".exos-agent/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
Exos Agent ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".exos-agent/plugins/my-plugin.ts"
...
```js title=".exos-agent/plugins/example.js"
...
- `client`: Un cliente SDK exos-agent para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar exos-agent.
...
```js title=".exos-agent/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "exos-agent"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio Exos Agent, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que exos-agent lea archivos `.env`:

```javascript title=".exos-agent/plugins/env-protection.js"
...
```javascript title=".exos-agent/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a exos-agent:

```ts title=".exos-agent/plugins/custom-tools.ts"
import { type Plugin, tool } from "@exos-agent/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que exos-agent puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para exos-agent junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://exos-agent.ai/docs/sdk) para obtener más detalles.
...
```ts title=".exos-agent/plugins/compaction.ts"
import type { Plugin } from "@exos-agent/plugin"
...
```ts title=".exos-agent/plugins/custom-compaction.ts"
import type { Plugin } from "@exos-agent/plugin"
