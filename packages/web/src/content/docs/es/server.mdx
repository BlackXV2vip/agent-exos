---
title: Servidor
description: Interactuar con el servidor exos-agent a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `exos-agent serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente exos-agent puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `exos-agent.local` |
...
Configure `EXOS_AGENT_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `exos-agent`, o configure `EXOS_AGENT_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `exos-agent serve` como a `exos-agent web`.
...
Cuando ejecuta `exos-agent`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor exos-agent para interactuar con exos-agent mediante programación.
:::

Esta arquitectura permite que exos-agent admita múltiples clientes y le permite interactuar con exos-agent mediante programación.

Puede ejecutar `exos-agent serve` para iniciar un servidor independiente. Si tienes el
exos-agent TUI ejecutándose, `exos-agent serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Exos Agent [IDE](/docs/ide).
...
El servidor exos-agent expone las siguientes API.
