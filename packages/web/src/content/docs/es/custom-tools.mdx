...
description: Cree herramientas que LLM pueda llamar en exos-agent.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de exos-agent como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.exos-agent/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/exos-agent/tools/`.
...
```ts title=".exos-agent/tools/database.ts" {1}
...
```ts title=".exos-agent/tools/math.ts"
import { tool } from "@exos-agent/plugin"
...
```ts title=".exos-agent/tools/bash.ts"
import { tool } from "@exos-agent/plugin"
...
```ts title=".exos-agent/tools/project.ts" {8}
import { tool } from "@exos-agent/plugin"
...
```python title=".exos-agent/tools/add.py"
import sys
...
```ts title=".exos-agent/tools/python-add.ts" {10}
import { tool } from "@exos-agent/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".exos-agent/tools/add.py")
