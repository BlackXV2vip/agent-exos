...
ผู้ให้บริการที่ได้รับความนิยมส่วนใหญ่จะถูกโหลดไว้ล่วงหน้าตามค่าเริ่มต้น หากคุณได้เพิ่มข้อมูลรับรองสำหรับผู้ให้บริการผ่านคำสั่ง `/connect` ข้อมูลเหล่านั้นจะพร้อมใช้งานเมื่อคุณเริ่ม Exos Agent
...
ต่อไปนี้เป็นโมเดลหลายรุ่นที่ทำงานได้ดีกับ Exos Agent โดยไม่เรียงลำดับเป็นพิเศษ (นี่ไม่ใช่รายการที่ครอบคลุมและไม่จำเป็นต้องเป็นข้อมูลล่าสุด):
...
ID แบบเต็มคือ `provider_id/model_id` ตัวอย่างเช่น หากคุณใช้ [Exos Agent Zen](/docs/zen) คุณจะใช้ `exos-agent/gpt-5.1-codex` สำหรับ GPT 5.1 Codex
...
โมเดลหลายตัวรองรับหลาย variants (รูปแบบย่อย) ที่มีการกำหนดค่าต่างกัน Exos Agent มาพร้อมกับ variants เริ่มต้นในตัวสำหรับผู้ให้บริการยอดนิยม

### Variants ในตัว (Built-in Variants)

Exos Agent มาพร้อมกับ variants เริ่มต้นสำหรับผู้ให้บริการหลายราย:
...
เมื่อ Exos Agent เริ่มทำงาน ระบบจะตรวจสอบรุ่นตามลำดับความสำคัญต่อไปนี้:

1. การตั้งค่าสถานะบรรทัดคำสั่ง `--model` หรือ `-m` รูปแบบจะเหมือนกับในไฟล์ปรับแต่ง: `provider_id/model_id`

2. รายการโมเดลในการกำหนดค่า Exos Agent
