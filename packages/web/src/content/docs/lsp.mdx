---
title: LSP Servers
description: Exos Agent integrates with your LSP servers.
---

Exos Agent can integrate with Language Server Protocol (LSP) servers to use diagnostics as feedback for the agent.

---

## Built-in

Exos Agent comes with several built-in LSP servers for popular languages:
...
:::note
You can disable automatic LSP server downloads by setting the `EXOS_AGENT_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When LSP is enabled and exos-agent opens a file, it:
...
You can enable and customize LSP servers through the `lsp` section in your exos-agent config.
...
```json title="exos-agent.json" {5-8}
{
  "$schema": "https://exos-agent.ai/config.json",
  "lsp": {
    "rust": {
...
```json title="exos-agent.json" {5-13}
{
  "$schema": "https://exos-agent.ai/config.json",
  "lsp": {
    "custom-lsp": {
