...
Agentferdigheter lar Exos Agent oppdage gjenbrukbare instruksjoner fra repo- eller hjemmekatalogen din.
...
exos-agent søker etter disse stedene:

- Prosjektkonfigurasjon: `.exos-agent/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/exos-agent/skills/<name>/SKILL.md`
...
For prosjektlokale stier går exos-agent opp fra din nåværende arbeidskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.exos-agent/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/exos-agent/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.exos-agent/skills/git-release/SKILL.md` slik:
...
exos-agent viser tilgjengelige ferdigheter i `skill`-verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved å bruke mønsterbaserte tillatelser i `exos-agent.json`:
...
**For innebygde agenter** (i `exos-agent.json`):
