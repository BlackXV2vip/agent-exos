---
title: Konfigurasjon
description: Ved å bruke exos-agent JSON-konfigurasjonen.
---

Du kan konfigurere Exos Agent ved å bruke en JSON konfigurasjonsfil.

---

## Format

Exos Agent støtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
For eksempel, hvis din globale konfigurasjon setter `theme: "exos-agent"` og `autoupdate: true`, og prosjektkonfigurasjonen setter `model: "anthropic/claude-sonnet-4-5"`, vil den endelige konfigurasjonen inkludere alle tre innstillingene.
...
1. **Ekstern konfigurasjon** (fra `.well-known/exos-agent`) - organisasjonsstandarder
2. **Global konfigurasjon** (`~/.config/exos-agent/exos-agent.json`) - brukerinnstillinger
3. **Egendefinert konfigurasjon** (`EXOS_AGENT_CONFIG` env var) - egendefinerte overstyringer
4. **Prosjektkonfigurasjon** (`exos-agent.json` i prosjekt) - prosjektspesifikke innstillinger
5. **`.exos-agent` kataloger** - agenter, kommandoer, plugins
6. **Inline config** (`EXOS_AGENT_CONFIG_CONTENT` env var) - kjøretidsoverstyringer
...
:::note
`.exos-agent` og `~/.config/exos-agent` katalogene bruker **flertallsnavn** på underkataloger: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkeltnavn (f.eks. `agent/`) støttes også for bakoverkompatibilitet.
:::
...
Organisasjoner kan tilby standardkonfigurasjon via `.well-known/exos-agent`-endepunktet. Dette hentes automatisk når du autentiserer med en leverandør som støtter det.
...
Plasser din globale Exos Agent-konfigurasjon i `~/.config/exos-agent/exos-agent.json`. Bruk global konfigurasjon for brukerspesifikke preferanser som temaer, leverandører eller nøkkelbindinger.
...
Legg til `exos-agent.json` i prosjektroten din. Prosjektkonfigurasjon har den høyeste prioritet blant standard config-filer - den overstyrer både globale og eksterne konfigurasjoner.

:::tip
Plasser prosjektspesifikk konfigurasjon i roten til prosjektet ditt.
:::

Når Exos Agent starter opp, ser den etter en konfigurasjonsfil i gjeldende katalog eller går opp til nærmeste Git-katalog.
...
Spesifiser en tilpasset konfigurasjonsfilbane ved å bruke miljøvariabelen `EXOS_AGENT_CONFIG`.
...
Spesifiser en tilpasset konfigurasjonskatalog ved å bruke `EXOS_AGENT_CONFIG_DIR`
miljøvariabel. Denne katalogen vil bli søkt etter agenter, kommandoer,
moduser og plugins akkurat som standard `.exos-agent` katalog, og bør
...
Den egendefinerte katalogen lastes inn etter den globale konfigurasjonen og `.exos-agent`-katalogene, slik at den **kan overstyre** innstillingene deres.

---

## Skjema

Konfigurasjonsfilen har et skjema som er definert i [**`exos-agent.ai/config.json`**](https://exos-agent.ai/config.json).
...
Du kan konfigurere serverinnstillinger for kommandoene `exos-agent serve` og `exos-agent web` gjennom alternativet `server`.
...
- `mdns` - Aktiver mDNS-tjenesteoppdagelse. Dette lar andre enheter på nettverket oppdage Exos Agent-serveren din.
- `mdnsDomain` - Egendefinert domenenavn for mDNS-tjeneste. Standard er `exos-agent.local`. Nyttig for å kjøre flere forekomster på samme nettverk.
...
Du kan konfigurere leverandørene og modellene du vil bruke i Exos Agent-konfigurasjonen gjennom alternativene `provider`, `model` og `small_model`.
...
Alternativet `small_model` konfigurerer en egen modell for lette oppgaver som tittelgenerering. Som standard prøver Exos Agent å bruke en rimeligere modell hvis en er tilgjengelig fra leverandøren din, ellers faller den tilbake til hovedmodellen din.
...
Du kan også definere agenter ved å bruke markdown-filer i `~/.config/exos-agent/agents/` eller `.exos-agent/agents/`. [Les mer her](/docs/agents).
...
Standardagenten må være en primæragent (ikke en underagent). Dette kan være en innebygd agent som `"build"` eller `"plan"`, eller en [egendefinert agent](/docs/agents) du har definert. Hvis den angitte agenten ikke eksisterer eller er en subagent, vil Exos Agent falle tilbake til `"build"` med en advarsel.

Denne innstillingen gjelder for alle grensesnitt: TUI, CLI (`exos-agent run`), skrivebordsapp og GitHub Action.
...
Du kan også definere kommandoer ved å bruke markdown-filer i `~/.config/exos-agent/commands/` eller `.exos-agent/commands/`. [Les mer her](/docs/commands).
...
Exos Agent vil automatisk laste ned eventuelle nye oppdateringer når den starter opp. Du kan deaktivere dette med alternativet `autoupdate`.
...
Som standard **tillater Exos Agent alle operasjoner** uten å kreve eksplisitt godkjenning. Du kan endre dette ved å bruke alternativet `permission`.
...
[Plugins](/docs/plugins) utvider Exos Agent med tilpassede verktøy, kroker og integrasjoner.

Plasser plugin-filer i `.exos-agent/plugins/` eller `~/.config/exos-agent/plugins/`. Du kan også laste inn plugins fra npm gjennom alternativet `plugin`.
...
Dette er nyttig når du vil begrense Exos Agent til kun å bruke spesifikke leverandører i stedet for å deaktivere dem én etter én.
