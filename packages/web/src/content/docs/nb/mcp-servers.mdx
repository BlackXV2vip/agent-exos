...
Du kan legge til eksterne verktøy i Exos Agent ved å bruke _Model Context Protocol_ eller MCP. Exos Agent støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Exos Agent Config](https://exos-agent.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="exos-agent.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/exos-agent`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
```jsonc title="exos-agent.jsonc" {15}
{
  "$schema": "https://exos-agent.ai/config.json",
  "mcp": {
    "my-local-mcp-server": {
...
Exos Agent håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Exos Agent:
...
Hvis serveren krever autentisering, vil Exos Agent be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `exos-agent mcp auth <server-name>`.
...
```json title="exos-agent.json" {7-11}
{
  "$schema": "https://exos-agent.ai/config.json",
  "mcp": {
    "my-oauth-server": {
...
```bash
exos-agent mcp auth my-oauth-server
```
...
```bash
exos-agent mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Exos Agent lagre tokens sikkert i `~/.local/share/exos-agent/mcp-auth.json`.
...
exos-agent mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i Exos Agent, sammen med innebygde verktøy. Så du kan administrere dem gjennom Exos Agent-konfigurasjonen som alle andre verktøy.
...
```json title="exos-agent.json" {11, 14-18}
{
  "$schema": "https://exos-agent.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```json title="exos-agent.json" {4-8}
{
  "$schema": "https://exos-agent.ai/config.json",
  "mcp": {
    "sentry": {
...
```bash
exos-agent mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Exos Agent til Sentry-kontoen din.
...
```json title="exos-agent.json" {4-7}
{
  "$schema": "https://exos-agent.ai/config.json",
  "mcp": {
    "context7": {
...
```json title="exos-agent.json" {7-9}
{
  "$schema": "https://exos-agent.ai/config.json",
  "mcp": {
    "context7": {
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Exos Agent-samtale oppført som en separat økt.
