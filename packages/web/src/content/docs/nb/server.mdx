---
title: Tjener
description: Samhandle med Exos Agent-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `exos-agent serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Exos Agent-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `exos-agent.local` |
...
Sett `EXOS_AGENT_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `exos-agent` som standard, eller sett `EXOS_AGENT_SERVER_USERNAME` for å overstyre det. Dette gjelder både `exos-agent serve` og `exos-agent web`.
...
Når du kjører `exos-agent` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk Exos Agent-serveren til å samhandle med Exos Agent programmatisk.
:::

Denne arkitekturen lar Exos Agent støtte flere klienter og lar deg samhandle med Exos Agent programmatisk.

Du kan kjøre `exos-agent serve` for å starte en frittstående server. Hvis du har
Exos Agent TUI kjører, vil `exos-agent serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Exos Agent [IDE](/docs/ide) plugins.
...
exos-agent-serveren viser følgende APIer.
