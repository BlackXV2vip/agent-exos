...
Opprett `.exos-agent/commands/test.md`:
...
Du kan legge til egendefinerte kommandoer gjennom Exos Agent-konfigurasjonen eller ved å lage markdown-filer i katalogen `commands/`.

---

### JSON

Bruk alternativet `command` i Exos Agent [config](/docs/config):
...
- Per prosjekt: `.exos-agent/commands/`
...
Exos Agent inkluderer flere innebygde kommandoer som `/init`, `/undo`, `/redo`, `/share`, `/help`; [finn ut mer](/docs/tui#commands).
