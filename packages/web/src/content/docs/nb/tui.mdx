---
title: TUI
description: Bruke exos-agent-terminalbrukergrensesnittet.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Exos Agent gir et interaktivt terminalgrensesnitt (TUI) for å jobbe med prosjektene dine med en LLM.

Å kjøre Exos Agent starter TUI for gjeldende katalog.
...
Når du bruker Exos Agent TUI, kan du skrive `/` etterfulgt av et kommandonavn for raskt å utføre handlinger. For eksempel:
...
Legg til en leverandør til Exos Agent. Lar deg velge fra tilgjengelige leverandører og legge til deres API-nøkler.
...
Avslutt Exos Agent. _Aliaser_: `/quit`, `/q`
...
Dette er atskilt fra `exos-agent.json`, som konfigurerer server-/kjøretidsoppførsel.
...
- `leader_timeout` - Styrer hvor lenge Exos Agent venter etter leader key. Standard er `2000`.
...
- `sound_pack` - Sound pack-ID som skal brukes. Standard er `exos-agent.default`.
