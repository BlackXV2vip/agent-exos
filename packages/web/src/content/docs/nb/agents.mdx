...
Det er to typer agenter i Exos Agent; primære agenter og underagenter.
...
Exos Agent kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
Exos Agent kommer med tre innebygde underagenter, **General**, **Explore** og **Scout**. Vi skal se på dette nedenfor.

---

## Innebygd

Exos Agent kommer med to innebygde primære agenter og tre innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `exos-agent.json`:
...
- Per prosjekt: `.exos-agent/agents/`
...
Hvis ingen temperatur er spesifisert, bruker Exos Agent modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale Exos Agent-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i Exos Agent-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [Exos Agent Zen](/docs/zen), vil du bruke `exos-agent/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `exos-agent models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/BlackXV2vip/agent-exos).
:::
