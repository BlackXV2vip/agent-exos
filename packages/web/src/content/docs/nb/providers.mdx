---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i Exos Agent.
---

import config from "../../../../config.mjs"
export const console = config.console

Exos Agent bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i Exos Agent-konfigurasjonen.
...
i `~/.local/share/exos-agent/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i Exos Agent
konfig.
...
Exos Agent Zen er en liste over modeller levert av Exos Agent-teamet som har vært
testet og verifisert for å fungere godt med Exos Agent. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med Exos Agent Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `ExosAgent Zen` og gå til [exos-agent.ai/auth](https://exos-agent.ai/zen).
...
Det fungerer som alle andre leverandører i Exos Agent og er helt valgfritt å bruke.

---

## Exos Agent Go

Exos Agent Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av Exos Agent-teamet som har vært
testet og verifisert for å fungere godt med Exos Agent.

1. Kjør kommandoen `/connect` i TUI, velg `ExosAgent Go`, og gå til [exos-agent.ai/auth](https://exos-agent.ai/zen).
...
Slik bruker du Amazon Bedrock med Exos Agent:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører Exos Agent:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `exos-agent.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i exos-agent støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere exos-agent til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at Exos Agent skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom Exos Agent-konfigurasjonen.
...
for å lagre token i Exos Agent auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
Exos Agent bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse Exos Agent
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`exos-agent.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [exos-agent-gitlab-auth](https://www.npmjs.com/package/exos-agent-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `exos-agent.json`:
...
Slik bruker du GitHub Copilot-abonnementet med Exos Agent:
...
Slik bruker du Google Vertex AI med Exos Agent:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører Exos Agent.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom Exos Agent, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [exos-agent-helicone-session](https://github.com/H2Shami/exos-agent-helicone-session) plugin for automatisk å logge hver Exos Agent-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver Exos Agent-samtale oppført som en separat økt.
...
Du kan konfigurere Exos Agent for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere Exos Agent for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere Exos Agent for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for Exos Agent. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/exos-agent) for detaljer.
:::
...
Slik bruker du Ollama Cloud med Exos Agent:
...
3. Kopier API-nøkkelen for bruk i Exos Agent.
...
6. **Viktig**: Før du bruker skymodeller i Exos Agent, må du hente modellinformasjonen lokalt:
...
Exos Agent Zen er en liste over testede og verifiserte modeller levert av Exos Agent-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>Exos Agent Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **Exos Agent Zen**.

   ```txt
   /connect
   ```

3. Skriv inn Exos Agent API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom Exos Agent-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom Exos Agent-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Exos Agent:
...
Du kan også tilpasse modeller gjennom Exos Agent-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med Exos Agent. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `exos-agent.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar Exos Agent forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `exos-agent auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk Exos Agent-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i Exos Agent-konfigurasjonen.
