---
title: الخادم
description: تفاعل مع خادم exos-agent عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `exos-agent serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل exos-agent استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `exos-agent.local` |
...
عيّن `EXOS_AGENT_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `exos-agent`، أو عيّن `EXOS_AGENT_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `exos-agent serve` و `exos-agent web`.
...
عند تشغيل `exos-agent` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم exos-agent للتفاعل مع exos-agent برمجيا.
:::

تتيح هذه البنية لـ exos-agent دعم عدة عملاء وتمكّنك من التفاعل مع exos-agent برمجيا.

يمكنك تشغيل `exos-agent serve` لبدء خادم مستقل. إذا كانت واجهة exos-agent في terminal (TUI)
قيد التشغيل، فسيبدأ `exos-agent serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات Exos Agent لـ [IDE](/docs/ide).
...
يعرض خادم exos-agent واجهات API التالية.
