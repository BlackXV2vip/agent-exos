---
title: TUI
description: استخدام واجهة المستخدم TUI في Exos Agent.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

يوفّر Exos Agent واجهة terminal تفاعلية (TUI) للعمل على مشاريعك باستخدام LLM.

يؤدي تشغيل Exos Agent إلى بدء واجهة TUI للدليل الحالي.
...
عند استخدام واجهة Exos Agent في terminal، يمكنك كتابة `/` متبوعة باسم أمر لتنفيذ الإجراءات بسرعة. مثلا:
...
أضف موفّرا إلى Exos Agent. يتيح لك اختيار أحد الموفّرين المتاحين وإضافة مفاتيح API الخاصة بهم.
...
اخرج من Exos Agent. _الأسماء المستعارة_: `/quit`, `/q`
...
هذا منفصل عن `exos-agent.json`، الذي يضبط سلوك الخادم ووقت التشغيل.
...
- `leader_timeout` - يتحكم في مدة انتظار Exos Agent بعد الضغط على leader key. القيمة الافتراضية `2000`.
...
- `sound_pack` - sound pack ID المراد استخدامه. القيمة الافتراضية `exos-agent.default`.
