...
مع Exos Agent يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم Exos Agent سمتنا `exos-agent`.
...
يأتي Exos Agent مع عدة سمات مدمجة.
...
- يريدون أن يطابق Exos Agent مظهر terminal لديهم
...
يدعم Exos Agent نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/exos-agent/themes/*.json` أو `$XDG_CONFIG_HOME/exos-agent/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.exos-agent/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.exos-agent/themes/*.json`
