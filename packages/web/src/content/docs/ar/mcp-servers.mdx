...
يمكنك إضافة أدوات خارجية إلى Exos Agent باستخدام _Model Context Protocol_، أو MCP. يدعم Exos Agent خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات Exos Agent](https://exos-agent.ai/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/exos-agent` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى Exos Agent تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم Exos Agent بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك Exos Agent المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `exos-agent mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن Exos Agent الرموز بشكل آمن في `~/.local/share/exos-agent/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل Exos Agent إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات Exos Agent مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط Exos Agent بحساب Sentry الخاص بك.
