...
أنشئ الملف `.exos-agent/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات Exos Agent أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ Exos Agent:
...
- على مستوى النظام: `~/.config/exos-agent/commands/`
- لكل مشروع: `.exos-agent/commands/`
...
يتضمن exos-agent عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
