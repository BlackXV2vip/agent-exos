...
تتيح مهارات الوكيل لـ Exos Agent اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث Exos Agent في هذه المواقع:

- إعدادات المشروع: `.exos-agent/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/exos-agent/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج Exos Agent صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.exos-agent/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/exos-agent/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.exos-agent/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض Exos Agent المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `exos-agent.json`:
...
**للوكلاء المدمجين** (في `exos-agent.json`):
