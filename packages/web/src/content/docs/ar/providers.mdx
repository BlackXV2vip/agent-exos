---
title: المزوّدون
description: استخدام أي موفّر LLM في Exos Agent.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Exos Agent ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Exos Agent.
...
في `~/.local/share/exos-agent/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Exos Agent.
...
Exos Agent Zen هي قائمة نماذج يوفّرها فريق Exos Agent وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Exos Agent. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Exos Agent Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر exos-agent، ثم انتقل إلى [exos-agent.ai/auth](https://exos-agent.ai/auth).
...
يعمل مثل أي مزوّد آخر في Exos Agent واستخدامه اختياري بالكامل.

---

## Exos Agent Go

Exos Agent Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق Exos Agent، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع Exos Agent.

1. شغّل الأمر `/connect` في TUI، واختر `ExosAgent Go`، ثم انتقل إلى [exos-agent.ai/auth](https://exos-agent.ai/zen).
...
لاستخدام Amazon Bedrock مع Exos Agent:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل exos-agent:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `exos-agent.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Exos Agent غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين exos-agent لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل exos-agent بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات exos-agent.
...
تخزين الرمز في مساحة تخزين مصادقة exos-agent.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم Exos Agent نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Exos Agent
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`exos-agent.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [exos-agent-gitlab-auth](https://www.npmjs.com/package/exos-agent-gitlab-auth).

##### التهيئة

خصّص عبر `exos-agent.json`:
...
لاستخدام اشتراك GitHub Copilot مع exos-agent:
...
لاستخدام Google Vertex AI مع Exos Agent:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر exos-agent، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [exos-agent-helicone-session](https://github.com/H2Shami/exos-agent-helicone-session) لتسجيل كل محادثة Exos Agent تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Exos Agent مدرجة كجلسة مستقلة.
...
يمكنك ضبط exos-agent لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط exos-agent لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط exos-agent لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Exos Agent. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/exos-agent) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع Exos Agent:
...
3. انسخ مفتاح API لاستخدامه في Exos Agent.
...
6. **مهم**: قبل استخدام نماذج السحابة في Exos Agent، يجب سحب معلومات النموذج محليا:
...
Exos Agent Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Exos Agent. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Exos Agent Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Exos Agent Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Exos Agent.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات exos-agent.
...
5. يمكنك أيضا تخصيصها عبر إعدادات exos-agent. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع Exos Agent:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات exos-agent. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع exos-agent. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `exos-agent.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ Exos Agent بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `exos-agent auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات exos-agent وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات exos-agent.
