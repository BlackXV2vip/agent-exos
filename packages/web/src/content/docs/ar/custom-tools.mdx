...
description: أنشئ أدوات يمكن لـ LLM استدعاؤها في exos-agent.
---

الأدوات المخصصة هي دوال تنشئها يمكن لـ LLM استدعاؤها أثناء المحادثات. تعمل جنبا إلى جنب مع [الأدوات المدمجة](/docs/tools) في exos-agent مثل `read` و`write` و`bash`.
...
- محليا بوضعها في مجلد `.exos-agent/tools/` داخل مشروعك.
- أو عالميا بوضعها في `~/.config/exos-agent/tools/`.
