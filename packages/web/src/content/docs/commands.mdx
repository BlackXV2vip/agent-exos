...
Create `.exos-agent/commands/test.md`:
...
You can add custom commands through the Exos Agent config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your Exos Agent [config](/docs/config):
...
- Per-project: `.exos-agent/commands/`
...
exos-agent includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
