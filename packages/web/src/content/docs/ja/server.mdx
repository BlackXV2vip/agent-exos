---
title: サーバー
description: HTTP 経由で Exos Agent サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`exos-agent serve` コマンドは、Exos Agent クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `exos-agent.local` |
...
HTTP 基本認証でサーバーを保護するには、`EXOS_AGENT_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `exos-agent` になるか、`EXOS_AGENT_SERVER_USERNAME` を設定してオーバーライドします。これは、`exos-agent serve` と `exos-agent web` の両方に当てはまります。
...
`exos-agent` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
Exos Agent サーバーを使用して、プログラムで Exos Agent と対話します。
:::

`exos-agent serve` を実行してスタンドアロンサーバーを起動できます。すでに Exos Agent TUI を実行している場合でも、`exos-agent serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、Exos Agent [IDE](/docs/ide) プラグインによって使用されます。
