---
title: プラグイン
description: Exos Agent を拡張する独自のプラグインを作成します。
---

プラグインを使用すると、さまざまなイベントにフックして動作をカスタマイズすることで Exos Agent を拡張できます。プラグインを作成して、新しい機能を追加したり、外部サービスと統合したり、Exos Agent のデフォルトの動作を変更したりできます。
...
- `.exos-agent/plugins/` - プロジェクトレベルのプラグイン
- `~/.config/exos-agent/plugins/` - グローバルプラグイン
...
**npm プラグイン** は、起動時に Bun を使用して自動的にインストールされます。パッケージとその依存関係は `~/.cache/exos-agent/node_modules/` にキャッシュされます。
...
1. グローバル設定 (`~/.config/exos-agent/exos-agent.json`)
2. プロジェクト設定 (`exos-agent.json`)
3. グローバルプラグインディレクトリ (`~/.config/exos-agent/plugins/`)
4. プロジェクトプラグインディレクトリ (`.exos-agent/plugins/`)
...
Exos Agent は起動時に `bun install` を実行してこれらをインストールします。プラグインとツールはそれらをインポートできるようになります。
...
- `client`: AI と対話するための Exos Agent SDK クライアント。
...
ここでは、Exos Agent を拡張するために使用できるプラグインの例をいくつか示します。
...
レベル: `debug`、`info`、`warn`、`error`。詳細については、[SDK ドキュメント](https://exos-agent.ai/docs/sdk) を参照してください。
...
Exos Agent が `.env` ファイルを読み取らないようにします。
...
プラグインは Exos Agent にカスタムツールを追加することもできます。
...
`tool` ヘルパーは、Exos Agent が呼び出すことができるカスタムツールを作成します。 Zod スキーマ関数を受け取り、次のようなツール定義を返します。

- `description`: ツールの機能
- `args`: ツールの引数の Zod スキーマ
- `execute`: ツールが呼び出されたときに実行される関数

カスタムツールは、組み込みツールと並行して Exos Agent で使用できます。
