...
Exos Agent Goは低価格のサブスクリプションで、**初月は5ドル**、その後は**月額10ドル**で、人気のオープンなコーディングモデルに安定してアクセスできます。

GoはExos Agentの他のプロバイダーと同様に機能します。Exos Agent GoをサブスクライブしてAPIキーを取得します。これは**完全に任意**であり、Exos Agentを使用するために必須ではありません。
...
:::tip
Exos Agentでうまく動作する一部のモデルとプロバイダーをテストしました。
:::
...
Exos Agent Goを使用すると、これらのモデルに**初月は5ドル**、その後は**月額10ドル**でアクセスできます。

---

## 仕組み

Exos Agent Goは、Exos Agentの他のプロバイダーと同様に機能します。

1. **<a href={console}>Exos Agent Zen</a>**にサインインし、GoをサブスクライブしてAPIキーをコピーします。
2. TUIで`/connect`コマンドを実行し、`ExosAgent Go`を選択して、APIキーを貼り付けます。
3. TUIで`/models`を実行すると、Goを通じて利用可能なモデルのリストが表示されます。

:::note
Exos Agent Goをサブスクライブできるのは、1つのワークスペースにつき1メンバーのみです。
:::
...
Exos Agent設定の[model id](/docs/config/#models)は、`exos-agent-go/<model-id>`という形式を使用します。たとえば、Kimi K3の場合は、設定で`exos-agent-go/kimi-k3`を使用します。
...
4. Exos Agentで他のプロバイダーも使用できるようにすることで、**ベンダーロックインをなくす**。
