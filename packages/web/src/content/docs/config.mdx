---
title: Config
description: Using the Exos Agent JSON config.
---

You can configure Exos Agent using a JSON config file.

---

## Format

Exos Agent supports both **JSON** and **JSONC** (JSON with Comments) formats.
...
1. **Remote config** (from `.well-known/exos-agent`) - organizational defaults
2. **Global config** (`~/.config/exos-agent/exos-agent.json`) - user preferences
3. **Custom config** (`EXOS_AGENT_CONFIG` env var) - custom overrides
4. **Project config** (`exos-agent.json` in project) - project-specific settings
5. **`.exos-agent` directories** - agents, commands, plugins
6. **Inline config** (`EXOS_AGENT_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/exos-agent/` on macOS) - admin-controlled
...
:::note
The `.exos-agent` and `~/.config/exos-agent` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Organizations can provide default configuration via the `.well-known/exos-agent` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
Place your global Exos Agent config in `~/.config/exos-agent/exos-agent.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/exos-agent/tui.json`.
...
Add `exos-agent.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When Exos Agent starts up, it first looks for a config file in the current directory, then traverses up to the nearest Git directory.
...
Specify a custom config file path using the `EXOS_AGENT_CONFIG` environment variable.
...
Specify a custom config directory using the `EXOS_AGENT_CONFIG_DIR`
environment variable. This directory will be searched for agents, commands,
modes, and plugins just like the standard `.exos-agent` directory, and should
...
The custom directory is loaded after the global config and `.exos-agent` directories, so it **can override** their settings.
...
Drop an `exos-agent.json` or `exos-agent.jsonc` file in the system managed config directory:

| Platform | Path                                     |
| -------- | ---------------------------------------- |
| macOS    | `/Library/Application Support/exos-agent/` |
| Linux    | `/etc/exos-agent/`                         |
| Windows  | `%ProgramData%\exos-agent`                 |

These directories require admin/root access to write, so users cannot modify them.

#### macOS managed preferences

On macOS, Exos Agent reads managed preferences from the `ai.exos-agent.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

Exos Agent checks these paths:

1. `/Library/Managed Preferences/<user>/ai.exos-agent.managed.plist`
2. `/Library/Managed Preferences/ai.exos-agent.managed.plist`

The plist keys map directly to `exos-agent.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.exos-agent.managed` PayloadType. The Exos Agent config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.exos-agent.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.exos-agent.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.exos-agent</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
The server/runtime config schema is defined in [**`exos-agent.ai/config.json`**](https://exos-agent.ai/config.json).

TUI config uses [**`exos-agent.ai/tui.json`**](https://exos-agent.ai/tui.json).
...
Use `EXOS_AGENT_TUI_CONFIG` to point to a custom TUI config file.

Set `attention.enabled` to turn on TUI desktop notifications and sounds. See [TUI attention](/docs/tui#attention).

Legacy `theme`, `keybinds`, and `tui` keys in `exos-agent.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `exos-agent serve` and `exos-agent web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your Exos Agent server.
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `exos-agent.local`. Useful for running multiple instances on the same network.
...
If not specified, Exos Agent will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your Exos Agent config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, Exos Agent tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
Use the `experimental.policies` option to allow or deny Exos Agent actions on configured resources. Currently, policies can control which providers Exos Agent may use.
...
Exos Agent normalizes image attachments before sending them to the model. By default, images are resized when they exceed `2000x2000` pixels or `5242880` base64 bytes.
...
If an image still cannot fit after resizing, Exos Agent omits oversized tool-result images or fails oversized user-provided images with an image size error.
...
You can also define agents using markdown files in `~/.config/exos-agent/agents/` or `.exos-agent/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, Exos Agent will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`exos-agent run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/exos-agent/commands/` or `.exos-agent/commands/`. [Learn more here](/docs/commands).
...
Exos Agent uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
Exos Agent will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
By default, exos-agent **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
[Plugins](/docs/plugins) extend Exos Agent with custom tools, hooks, and integrations.

Place plugin files in `.exos-agent/plugins/` or `~/.config/exos-agent/plugins/`. You can also load plugins from npm through the `plugin` option.
...
This is useful when you want to restrict Exos Agent to only use specific providers rather than disabling them one by one.
