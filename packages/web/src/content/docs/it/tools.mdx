...
Gli strumenti permettono all'LLM di eseguire azioni nel tuo codebase. Exos Agent include un set di strumenti integrati, ma puoi estenderlo con [strumenti personalizzati](/docs/custom-tools) o con [server MCP](/docs/mcp-servers).
...
Ecco tutti gli strumenti integrati disponibili in Exos Agent.
...
:::note
Questo strumento e' disponibile solo quando `EXOS_AGENT_EXPERIMENTAL_LSP_TOOL=true` (oppure `EXOS_AGENT_EXPERIMENTAL=true`).
:::
...
:::note
Questo strumento e' disponibile solo quando usi il provider Exos Agent o quando la variabile d'ambiente `EXOS_AGENT_ENABLE_EXA` e' impostata a un valore truthy (ad esempio `true` o `1`).

Per abilitarlo all'avvio di Exos Agent:
