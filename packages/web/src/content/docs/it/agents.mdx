...
In Exos Agent esistono due tipi di agenti: agenti primari e subagenti.
...
Exos Agent include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
Exos Agent include tre subagenti integrati: **General**, **Explore** e **Scout**. Li vediamo sotto.

---

## Integrati

Exos Agent include due agenti primari integrati e tre subagenti integrati.
...
Un agente in sola lettura per la ricerca su documentazione esterna e dipendenze. Usalo quando devi clonare il repository di una dipendenza nella cache gestita di Exos Agent, ispezionare il codice sorgente di una libreria o confrontare il codice locale con implementazioni upstream senza modificare il tuo workspace.
...
Configura gli agenti nel file `exos-agent.json`:
...
- Globale: `~/.config/exos-agent/agents/`
- Per progetto: `.exos-agent/agents/`
...
Se non specifichi una temperature, Exos Agent usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di Exos Agent sia per la config specifica del progetto.
...
L'ID modello nella configurazione Exos Agent usa il formato `provider/model-id`. Per esempio, se stai usando [Exos Agent Zen](/docs/zen), useresti `exos-agent/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `exos-agent models` per vedere la lista dei modelli disponibili.
:::
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/BlackXV2vip/agent-exos).
:::
