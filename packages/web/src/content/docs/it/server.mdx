---
title: Server
description: Interagisci con il server exos-agent via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `exos-agent serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client exos-agent.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `exos-agent.local` |
...
Imposta `EXOS_AGENT_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `exos-agent`, oppure imposta `EXOS_AGENT_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `exos-agent serve` sia per `exos-agent web`.
...
Quando esegui `exos-agent` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server exos-agent per interagire con exos-agent in modo programmatico.
:::

Questa architettura permette a exos-agent di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `exos-agent serve` per avviare un server standalone. Se la TUI di exos-agent e gia in esecuzione, `exos-agent serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di Exos Agent.
...
Il server exos-agent espone le seguenti API.
