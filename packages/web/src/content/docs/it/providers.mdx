---
title: Provider
description: Usa qualsiasi provider LLM in Exos Agent.
---

import config from "../../../../config.mjs"
export const console = config.console

Exos Agent usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di Exos Agent.
...
Exos Agent Zen e una lista di modelli forniti dal team Exos Agent, testati e
verificati per funzionare bene con Exos Agent. [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da Exos Agent Zen.
:::

1. Esegui `/connect` nella TUI, seleziona exos-agent e vai su [exos-agent.ai/auth](https://exos-agent.ai/auth).
...
Funziona come qualsiasi altro provider in Exos Agent ed e totalmente opzionale.
...
Per usare Amazon Bedrock con Exos Agent:
...
2. **Configura l'autenticazione** usando uno dei seguenti metodi:
   ...
   ***
   ...
   Imposta una di queste variabili d'ambiente mentre esegui exos-agent:
   ...
   ***
   ...
   Per configurazione specifica del progetto o persistente, usa `exos-agent.json`:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in Exos Agent non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
Puoi configurare exos-agent per utilizzare modelli locali tramite [Atomic Chat](https://atomic.chat), un'applicazione desktop che esegue LLM locali dietro un server API compatibile OpenAI (endpoint predefinito `http://127.0.0.1:1337/v1`).
...
2. Vai su [Azure AI Foundry](https://ai.azure.com/) e fai il deploy di un modello.
   ...
   :::note
   Il nome del deployment deve corrispondere al nome del modello affinché exos-agent funzioni correttamente.
   :::
...
5. Esegui il comando `/models` per selezionare un modello.
   ...
   Puoi anche aggiungere modelli tramite la tua configurazione di exos-agent.
...
salvare il token nello storage di auth di exos-agent.
:::

##### GitLab Self-Hosted

:::note[nota di compliance]
Exos Agent usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare Exos Agent
a usare solo la tua istanza GitLab self-hosted, aggiungi quanto segue al tuo
file `exos-agent.json`. Si raccomanda anche di disabilitare la condivisione sessioni.
...
Maggior documentazione sulla homepage di [exos-agent-gitlab-auth](https://www.npmjs.com/package/exos-agent-gitlab-auth).

##### Configurazione

Personalizza tramite `exos-agent.json`:
...
Per usare il tuo abbonamento GitHub Copilot con exos-agent:
...
Per usare Google Vertex AI con Exos Agent:
...
2. Imposta le variabili d'ambiente richieste:
   ...
   Impostale mentre esegui exos-agent.
...
Nel caso vedessi una funzionalità o un modello di Helicone che non viene configurato automaticamente da exos-agent, puoi sempre configurarlo tu stesso.
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [exos-agent-helicone-session](https://github.com/H2Shami/exos-agent-helicone-session) per loggare automaticamente ogni conversazione di Exos Agent come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di Exos Agent elencata come sessione separata.
...
Puoi configurare exos-agent per usare modelli locali tramite l'utility llama-server di [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puoi configurare exos-agent per usare modelli locali tramite LM Studio.
...
Puoi configurare exos-agent per usare modelli locali tramite Ollama.

:::tip
Ollama può auto-configurarsi per Exos Agent. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/exos-agent) per dettagli.
:::
...
Per usare Ollama Cloud con Exos Agent:
...
3. Copia la chiave API da usare in Exos Agent.
...
6. **Importante**: Prima di usare modelli cloud in Exos Agent, devi scaricare le informazioni del modello localmente:
...
Exos Agent Zen è una lista di modelli testati e verificati forniti dal team Exos Agent. [Scopri di più](/docs/zen).

1. Accedi a **<a href={console}>Exos Agent Zen</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **Exos Agent Zen**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di Exos Agent.
...
4. Molti modelli OpenRouter sono precaricati di default, esegui il comando `/models` per selezionare quello che vuoi.
   ...
   Puoi anche aggiungere modelli addizionali tramite la tua configurazione di exos-agent.
...
5. Puoi anche personalizzarli tramite la configurazione di exos-agent. Ecco un esempio di specifica di un provider
...
Per usare le [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) con Exos Agent:
...
Puoi anche personalizzare i modelli tramite la tua configurazione di exos-agent. Ecco un esempio di come specificare l'ordine di routing dei provider.
...
:::tip
Puoi usare qualsiasi provider OpenAI-compatible con exos-agent. La maggior parte dei provider AI moderni offre API OpenAI-compatible.
:::
...
4. Crea o aggiorna il tuo file `exos-agent.json` nella directory del progetto:
...
I campi `limit` permettono a Exos Agent di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
...
1. **Controlla il setup auth**: Esegui `exos-agent auth list` per vedere se le credenziali
...
2. Per provider personalizzati, controlla la config di exos-agent e:
   - Assicurati che l'ID provider usato nel comando `/connect` corrisponda all'ID nella tua config exos-agent.
