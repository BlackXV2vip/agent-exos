---
title: Plugin
description: Scrivi plugin per estendere Exos Agent.
---

I plugin ti permettono di estendere Exos Agent agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Exos Agent.
...
- `.exos-agent/plugins/` - plugin a livello progetto
- `~/.config/exos-agent/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/exos-agent/node_modules/`.
...
1. Config globale (`~/.config/exos-agent/exos-agent.json`)
2. Config di progetto (`exos-agent.json`)
3. Directory plugin globale (`~/.config/exos-agent/plugins/`)
4. Directory plugin di progetto (`.exos-agent/plugins/`)
...
Exos Agent esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di exos-agent per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere exos-agent.
...
:::note
Se usi l'app desktop di Exos Agent, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a exos-agent di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a exos-agent:
...
L'helper `tool` crea uno strumento personalizzato che exos-agent può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in exos-agent insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://exos-agent.ai/docs/sdk) per i dettagli.
