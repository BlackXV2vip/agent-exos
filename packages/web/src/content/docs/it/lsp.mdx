---
title: Server LSP
description: Exos Agent si integra con i tuoi server LSP.
---

Exos Agent può integrarsi con server Language Server Protocol (LSP) per usare la diagnostica come feedback per l'agente.

---

## Integrati

Exos Agent include diversi server LSP integrati per linguaggi popolari:
...
:::note
Puoi disabilitare il download automatico dei server LSP impostando la variabile d'ambiente `EXOS_AGENT_DISABLE_LSP_DOWNLOAD` a `true`.
:::
...
Quando LSP è abilitato e exos-agent apre un file:
...
Puoi abilitare e personalizzare i server LSP tramite la sezione `lsp` nella configurazione di exos-agent.
