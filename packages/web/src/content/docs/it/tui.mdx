---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di Exos Agent.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Exos Agent fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire Exos Agent avvia la TUI per la directory corrente.
...
Quando usi la TUI di Exos Agent, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a Exos Agent. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da Exos Agent. _Alias_: `/quit`, `/q`
...
Questo è separato da `exos-agent.json`, che configura il comportamento di server/runtime.
...
- `leader_timeout` - Controlla per quanto tempo Exos Agent attende dopo il leader key. Default: `2000`.
...
- `sound_pack` - ID del sound pack da usare. Default: `exos-agent.default`.
