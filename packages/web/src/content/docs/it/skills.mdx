...
Le skill degli agenti permettono a Exos Agent di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Exos Agent cerca in queste posizioni:

- Config di progetto: `.exos-agent/skills/<name>/SKILL.md`
- Config globale: `~/.config/exos-agent/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Exos Agent risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.exos-agent/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/exos-agent/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.exos-agent/skills/git-release/SKILL.md` cosi':
...
Exos Agent elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `exos-agent.json`:
...
**Per agenti integrati** (in `exos-agent.json`):
