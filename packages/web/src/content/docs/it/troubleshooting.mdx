...
Per diagnosticare problemi con Exos Agent, inizia controllando i log e i dati locali che salva su disco.
...
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\exos-agent\log`

I file di log hanno nomi con timestamp (ad es. `2025-01-09T123456.log`) e vengono conservati i 10 file di log piu recenti.

Puoi impostare il livello di log con l'opzione a riga di comando `--log-level` per ottenere informazioni di debug piu dettagliate. Per esempio: `exos-agent --log-level DEBUG`.

---

## Archiviazione

exos-agent salva i dati delle sessioni e altri dati dell'applicazione su disco in:

- **macOS/Linux**: `~/.local/share/exos-agent/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\exos-agent`
...
Exos Agent Desktop esegue in background un server locale di Exos Agent (il sidecar `exos-agent-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- Solo macOS: menu `ExosAgent` -> **Reload Webview** (utile se l'interfaccia e vuota o bloccata).
...
- **macOS/Linux** (installazioni vecchie): `~/.local/share/exos-agent/exos-agent.jsonc`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\exos-agent\exos-agent.jsonc`
...
Exos Agent puo anche caricare plugin locali dal disco. Spostali temporaneamente altrove (o rinomina la cartella) e riavvia l'app desktop:

- **Plugin globali**
  - **macOS/Linux**: `~/.config/exos-agent/plugins/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\exos-agent\plugins`
...
Se disabilitare i plugin non aiuta (o l'installazione di un plugin e bloccata), svuota la cache in modo che Exos Agent possa ricostruirla.

1. Chiudi completamente Exos Agent Desktop.
2. Elimina la directory della cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/exos-agent`
- **Linux**: elimina `~/.cache/exos-agent` (oppure esegui `rm -rf ~/.cache/exos-agent`)
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.cache\exos-agent`

3. Riavvia Exos Agent Desktop.

---

### Risolvi problemi di connessione al server

Exos Agent Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Se il tuo `exos-agent.json(c)` contiene una sezione `server`, rimuovila temporaneamente e riavvia l'app desktop.

#### Controlla le variabili d'ambiente

Se hai `EXOS_AGENT_PORT` impostato nell'ambiente, l'app desktop provera a usare quella porta per il server locale.

- Rimuovi `EXOS_AGENT_PORT` (o scegli una porta libera) e riavvia.
...
Su Windows, Exos Agent Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.

---

### Windows: problemi generali di prestazioni

Se riscontri prestazioni lente, problemi di accesso ai file o problemi del terminale su Windows, prova a usare [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL fornisce un ambiente Linux che funziona in modo piu fluido con le funzionalita di Exos Agent.

---

### Notifiche non visualizzate

Exos Agent Desktop mostra le notifiche di sistema solo quando:

- le notifiche sono abilitate per Exos Agent nelle impostazioni del sistema operativo, e
...
1. Chiudi Exos Agent Desktop.
2. Trova ed elimina questi file (si trovano nella directory dati dell'app Exos Agent Desktop):

- `exos-agent.settings.dat` (desktop default server URL)
- `exos-agent.global.dat` e `exos-agent.workspace.*.dat` (stato dell'interfaccia come server/progetti recenti)
...
Se riscontri problemi con Exos Agent:
...
Per capire a quali modelli hai accesso, esegui `exos-agent models`
...
2. Se il problema persiste, prova a cancellare la configurazione salvata:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.local\share\exos-agent`
...
Se incontri errori nelle chiamate API, potrebbe dipendere da pacchetti provider non aggiornati. exos-agent installa dinamicamente i pacchetti provider (OpenAI, Anthropic, Google, ecc.) quando servono e li mette in cache localmente.
...
1. Svuota la cache dei pacchetti provider:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.cache\exos-agent`

2. Riavvia exos-agent per reinstallare i pacchetti provider piu recenti

Questo forzera exos-agent a scaricare le versioni piu recenti dei pacchetti provider, cosa che spesso risolve problemi di compatibilita con parametri dei modelli e cambiamenti delle API.
...
exos-agent rilevera se stai usando Wayland e preferira `wl-clipboard`; altrimenti provera a trovare gli strumenti per gli appunti nell'ordine: `xclip` e `xsel`.
