...
Exos Agent Go è un abbonamento a basso costo — **5 $ per il primo mese**, poi **10 $/mese** — che ti offre un accesso affidabile ai popolari modelli di programmazione aperti.

Go funziona come qualsiasi altro provider in Exos Agent. Ti abboni a Exos Agent Go e
ottieni la tua chiave API. È **completamente facoltativo** e non hai bisogno di usarlo per
utilizzare Exos Agent.
...
Exos Agent Go ti dà accesso a questi modelli a **5 $ per il primo mese**, poi a **10 $/mese**.

---

## Come funziona

Exos Agent Go funziona come qualsiasi altro provider in Exos Agent.

1. Accedi a **<a href={console}>Exos Agent Zen</a>**, ti abboni a Go e
   copi la tua chiave API.
2. Esegui il comando `/connect` nella TUI, selezioni `ExosAgent Go` e incolli
   la tua chiave API.
3. Esegui `/models` nella TUI per vedere l'elenco dei modelli disponibili tramite Go.

:::note
Solo un membro per workspace può abbonarsi a Exos Agent Go.
:::
...
Exos Agent Go include i seguenti limiti:
...
Il [model id](/docs/config/#models) nella tua Exos Agent config
utilizza il formato `exos-agent-go/<model-id>`. Ad esempio, per Kimi K3, useresti
`exos-agent-go/kimi-k3` nella tua configurazione.
...
Abbiamo creato Exos Agent Go per:
...
4. Non avere **nessun lock-in**, permettendoti di utilizzare anche qualsiasi altro provider con Exos Agent.
