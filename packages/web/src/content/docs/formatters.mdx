---
title: Formatters
description: Exos Agent uses language specific formatters.
---

Exos Agent can format files after they are written or edited using language-specific formatters. Formatters are disabled by default; enable them in your config before Exos Agent will run them.

---

## Built-in

Exos Agent comes with several built-in formatters for popular languages and frameworks. Below is a list of the formatters, supported file extensions, and commands or config options it needs.
...
When formatters are enabled, Exos Agent will use `prettier` for matching files if your project has `prettier` in `package.json`.

---

## How it works

When Exos Agent writes or edits a file and formatters are enabled, it:
...
You can enable and customize formatters through the `formatter` section in your Exos Agent config.
