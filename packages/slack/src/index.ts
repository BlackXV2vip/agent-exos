...
import { createExosAgent, type ToolPart } from "@exos-agent/sdk"
...
console.log("🚀 Starting exos-agent server...")
const exos-agent = await createExosAgent({
  port: 0,
})
console.log("✅ ExosAgent server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await exos-agent.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new exos-agent session...")
    const { client, server } = exos-agent
    ...
    console.log("✅ Created exos-agent session:", createResult.data.id)
  ...
  console.log("📤 ExosAgent response:", JSON.stringify(result, null, 2))
