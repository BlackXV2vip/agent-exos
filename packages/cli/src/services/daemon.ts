...
import { createExosAgentClient } from "@exos-agent/sdk/v2/client"
import { ServerAuth } from "@exos-agent/server/auth"
...
export interface Interface {
  readonly client: () => Effect.Effect<ReturnType<typeof createExosAgentClient>, unknown>
...
export class Service extends Context.Service<Service, Interface>()("@exos-agent/cli/Daemon") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const createClient = Effect.fnUntraced(function* (url: string) {
      return createExosAgentClient({ baseUrl: url, headers: ServerAuth.headers({ password: yield* password() }) })
    })
    ...
    const client = Effect.fn("cli.daemon.client")(function* () {
      ...
      return createExosAgentClient({ baseUrl: connection.url, headers: connection.headers })
    })
