  ...
  export const invite = fn(
    ...
    async ({ email, role, monthlyLimit }) => {
      ...
      try {
        ...
        const { InviteEmail } = await import("@exos-agent/console-mail/InviteEmail.jsx")
        await AWS.sendEmail({
          to: email,
          subject: `You've been invited to join the ${emailInfo.workspaceName} workspace on ExosAgent`,
          body: render(
            // @ts-ignore
            InviteEmail({
              inviter: emailInfo.inviterEmail,
              assetsUrl: `https://exos-agent.ai/email`,
