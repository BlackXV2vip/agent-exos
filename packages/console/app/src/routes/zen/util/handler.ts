...
import { and, Database, eq, isNull, lt, or, sql } from "@exos-agent/console-core/drizzle/index.js"
import { KeyTable } from "@exos-agent/console-core/schema/key.sql.js"
import { BillingTable, LiteTable, SubscriptionTable, UsageTable } from "@exos-agent/console-core/schema/billing.sql.js"
import { centsToMicroCents } from "@exos-agent/console-core/util/price.js"
import { getMonthlyBounds, getWeekBounds } from "@exos-agent/console-core/util/date.js"
import { Identifier } from "@exos-agent/console-core/identifier.js"
import { Billing } from "@exos-agent/console-core/billing.js"
import { Actor } from "@exos-agent/console-core/actor.js"
import { WorkspaceTable } from "@exos-agent/console-core/schema/workspace.sql.js"
import { ZenData } from "@exos-agent/console-core/model.js"
import { Subscription } from "@exos-agent/console-core/subscription.js"
import { BlackData } from "@exos-agent/console-core/black.js"
import { UserTable } from "@exos-agent/console-core/schema/user.sql.js"
import { ModelTable } from "@exos-agent/console-core/schema/model.sql.js"
import { ProviderTable } from "@exos-agent/console-core/schema/provider.sql.js"
...
import { LiteData } from "@exos-agent/console-core/lite.js"
...
import { Workspace } from "@exos-agent/console-core/workspace.js"
...
export async function handler(
  ...
  try {
    ...
    const sessionId = input.request.headers.get("x-exos-agent-session") ?? ""
    const requestId = input.request.headers.get("x-exos-agent-request") ?? ""
    const ocClient = input.request.headers.get("x-exos-agent-client") ?? ""
    const projectId = input.request.headers.get("x-exos-agent-project") ?? ""
      ...
      if (!allowedRegions?.includes("unavailable"))
        throw new RegionError(
          t("zen.api.error.regionNotAllowed", {
            consoleGoUrl: `https://exos-agent.ai/workspace/${authInfo.workspaceID}/go`,
          }),
        )
    }
    */
    ...
    const retriableRequest = async (retry: RetryOptions = { excludeProviders: [], retryCount: 0 }) => {
      ...
      const res = await fetchWithRetryableStatus(
        reqUrl,
        {
          ...
          headers: (() => {
            ...
            headers.delete("x-exos-agent-request")
            headers.delete("x-exos-agent-session")
            headers.delete("x-exos-agent-project")
            headers.delete("x-exos-agent-client")
            return headers
          })(),
          body: reqBody,
      ...
      if (isNewInference) {
        const resEndpointId = res.headers.get("x-exos-agent-endpoint-id")
        const resEndpointModelId = res.headers.get("x-exos-agent-upstream-model-id")
  ...
  function validateBilling(authInfo: AuthInfo, modelInfo: ModelInfo): BillingSource {
    ...
    if (opts.modelList === "lite" && authInfo.billing.lite && authInfo.lite) {
      try {
        const consoleGoUrl = `https://exos-agent.ai/workspace/${authInfo.workspaceID}/go`
    ...
    const billingUrl = `https://exos-agent.ai/workspace/${authInfo.workspaceID}/billing`
    const membersUrl = `https://exos-agent.ai/workspace/${authInfo.workspaceID}/members`
