import { Message, Model, Part, Session, SessionStatus, SnapshotFileDiff, UserMessage } from "@exos-agent/sdk/v2"
import { SessionTurn } from "@exos-agent/session-ui/session-turn"
import { SessionReview } from "@exos-agent/session-ui/session-review"
import { DataProvider } from "@exos-agent/session-ui/context"
import { FileComponentProvider } from "@exos-agent/ui/context/file"
import { WorkerPoolProvider } from "@exos-agent/ui/context/worker-pool"
import { createAsync, query, useParams } from "@solidjs/router"
import { createMemo, createSignal, ErrorBoundary, For, Match, Show, Switch } from "solid-js"
import { Share } from "~/core/share"
import { Logo, Mark } from "@exos-agent/ui/logo"
import { IconButton } from "@exos-agent/ui/icon-button"
import { ProviderIcon } from "@exos-agent/ui/provider-icon"
import { iife } from "@exos-agent/core/util/iife"
import { Binary } from "@exos-agent/core/util/binary"
import { NamedError } from "@exos-agent/core/util/error"
import { DateTime } from "luxon"
import { createStore } from "solid-js/store"
import NotFound from "../[...404]"
import { Tabs } from "@exos-agent/ui/tabs"
import { MessageNav } from "@exos-agent/session-ui/message-nav"
import { FileSSR } from "@exos-agent/session-ui/file-ssr"
...
const ClientOnlyWorkerPoolProvider = clientOnly(() =>
  import("@exos-agent/session-ui/pierre/worker").then((m) => ({
      ...
      <Show when={data()}>
        {(data) => {
          ...
          const ogImage = createMemo(() => {
            ...
            return `https://social-cards.sst.dev/exos-agent-share/${encodedTitle}.png?model=${modelParam}&version=${version}&id=${data().shareID}`
          })

          return (
            <>
              <Show when={info().title}>
                <Title>{info().title} | ExosAgent</Title>
              </Show>
              <Meta name="description" content="exos-agent - The AI coding agent built for the terminal." />
                        ...
                        <div class="relative bg-background-stronger w-screen h-screen overflow-hidden flex flex-col">
                          <header class="h-12 px-6 py-2 flex items-center justify-between self-stretch bg-background-base border-b border-border-weak-base">
                            ...
                            <div class="flex gap-3 items-center">
                              <IconButton
                                as={"a"}
                                href="https://github.com/BlackXV2vip/agent-exos"
                                target="_blank"
                                icon="github"
                                variant="ghost"
                              />
                              <IconButton
                                as={"a"}
                                href="https://exos-agent.ai/discord"
                                target="_blank"
                                icon="discord"
                                variant="ghost"
                              />
