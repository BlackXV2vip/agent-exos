...
//   - EXOS_AGENT_DISABLE_PROJECT_CONFIG : skip walking up for exos-agent.json
...
//
// Today only `exos-agent.run` is fully wired. The shape supports adding more
// builders (`exos-agent.serve(opts)`, `exos-agent.acp(opts)`, `exos-agent.auth(...)`)
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("exos-agent.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const startRun = Effect.fn("exos-agent.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("exos-agent.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
    ...
    const acp = Effect.fn("exos-agent.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
...
// resources (e.g. `exos-agent.serve`) without an extra `Effect.scoped` wrapper —
