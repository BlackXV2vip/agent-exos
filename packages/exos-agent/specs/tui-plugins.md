...
- Author package entrypoint is `@exos-agent/plugin/tui`.
...
- Import types from `@exos-agent/plugin/tui`.
- `@exos-agent/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@exos-agent/plugin/tui"
...
When `plugin` entries exist in a writable `.exos-agent` dir or `EXOS_AGENT_CONFIG_DIR`, Exos Agent installs `@exos-agent/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@exos-agent/plugin/tui`.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@exos-agent/plugin/tui`.
