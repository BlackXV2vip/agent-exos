...
import { InstallationVersion } from "@exos-agent/core/installation/version"
import { AppNodeBuilder } from "@exos-agent/core/effect/app-node-builder"
import type { AssistantMessage, Message, ExosAgentClient, SessionMessageResponse } from "@exos-agent/sdk/v2"
...
export const AuthMethodID = "exos-agent-login"
...
export class Service extends Context.Service<Service, Interface>()("@exos-agent/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `exos-agent auth login` in the terminal",
      name: "Login with exos-agent",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: ExosAgentClient) {
...
function makeUsageService(sdk: ExosAgentClient) {
...
async function loadDirectorySnapshot(sdk: ExosAgentClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, exos-agent provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const exosAgentProvider = providers[ProviderV2.ID.make("exos-agent")]
  const exosAgentModel = exosAgentProvider ? Provider.sort(Object.values(exosAgentProvider.models))[0] : undefined
  if (exosAgentProvider && exosAgentModel) return { providerID: exosAgentProvider.id, modelID: exosAgentModel.id }
...
function promptErrorMessage(error: AssistantError) {
  ...
  return "ExosAgent prompt failed"
}
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "ExosAgent service failure", service })
}
