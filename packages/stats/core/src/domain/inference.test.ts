...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve exos-agent route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "exos-agent")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "exos-agent")).toBe("openai")
    expect(statProvider("big-pickle", "", "exos-agent")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "exos-agent"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep exos-agent as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "exos-agent"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "exos-agent"))).toMatchObject([{ provider: "unknown" }])
  })

  test("geo aggregates never keep exos-agent or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "exos-agent"), country: "US" })).toMatchObject([
