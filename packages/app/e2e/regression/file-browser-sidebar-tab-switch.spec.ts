...
const directory = "C:/ExosAgent/FileBrowserSidebar"
...
async function setup(page: Page) {
  await mockExosAgentServer(page, {
    ...
    provider: {
      ...
      default: { providerID: "exos-agent", modelID: "test" },
    },
    sessions: [
      {
        id: sessionID,
  ...
  await page.addInitScript(
    ({ directory, server, sessionID }) => {
      ...
      localStorage.setItem(
        "exos-agent.global.dat:layout",
        JSON.stringify({ review: { diffStyle: "split", panelOpened: true } }),
      )
      localStorage.setItem(
        "exos-agent.global.dat:review-panel-v2",
        JSON.stringify({ sidebarOpened: true, sidebarWidth: 240, expandMode: "collapse" }),
      )
      localStorage.setItem(
        "exos-agent.window.browser.dat:tabs",
