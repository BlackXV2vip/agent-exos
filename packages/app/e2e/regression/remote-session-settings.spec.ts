...
async function configureServers(page: Page, tabs: { type: "session"; server: string; sessionId: string }[] = []) {
  await page.addInitScript(
    ({ serverB, tabs }) => {
      ...
      localStorage.setItem("exos-agent.global.dat:server", JSON.stringify({ list: [serverB] }))
      localStorage.setItem("exos-agent.window.browser.dat:tabs", JSON.stringify(tabs))
    },
    { serverB, tabs },
  )
}
