...
import { SessionStatusEvent } from "@exos-agent/schema/session-status-event"
import { SessionV1 } from "@exos-agent/schema/session-v1"
import type {
  AssistantMessage,
  GlobalEvent,
  Message,
...
import { mockExosAgentServer } from "../../utils/mock-server"
import { installSseTransport } from "../../utils/sse-transport"
import { expectSessionTitle } from "../../utils/waits"

export const directory = "C:/ExosAgent/TimelineStability"
...
export const model = { providerID: "exos-agent", modelID: "claude-opus-4-6", variant: "max" }
...
export async function setupTimeline(
  ...
  await mockExosAgentServer(page, {
    directory,
    project: project(),
    provider: provider(),
    sessions,
  ...
  if (input.locale) {
    await page.addInitScript((locale) => {
      localStorage.setItem("exos-agent.global.dat:language", JSON.stringify({ locale }))
    }, input.locale)
  }
...
function provider() {
  return {
    ...
    connected: ["exos-agent"],
    default: { providerID: "exos-agent", modelID: "claude-opus-4-6" },
  }
}
