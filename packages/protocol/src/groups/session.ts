...
import { PromptInput } from "@exos-agent/schema/prompt-input"
import { Session } from "@exos-agent/schema/session"
import { Project } from "@exos-agent/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@exos-agent/schema/schema"
...
import { SessionEvent } from "@exos-agent/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this ExosAgent process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this ExosAgent process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
