...
export function registerWslIpcHandlers(controller: WslServersController) {
  ...
  ipcMain.handle("wsl-servers-install-exos-agent", (_event: IpcMainInvokeEvent, name: string) =>
    controller.installExosAgent(requireWslIpcString("distro", name)),
  )
...
function registerUnavailableWslIpcHandlers() {
  ...
  ipcMain.handle("wsl-servers-install-exos-agent", unavailable)
