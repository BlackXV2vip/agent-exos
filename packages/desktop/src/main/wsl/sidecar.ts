...
import { type WslCommandLine, resolveWslExosAgent, shellEscape, wslArgs } from "./runtime"
...
export async function spawnWslSidecar(
  ...
  const exos-agent = await resolveWslExosAgent(distro)
  if (!exos-agent) throw new Error(`ExosAgent is not installed in ${distro}`)

  const port = await allocatePort()
  const password = randomUUID()
  const username = "exos-agent"
  const script = [
    "set -euo pipefail",
    'cd "$HOME" || cd /',
    ...
    "export PATH",
    "export WSLENV=",
    "export EXOS_AGENT_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export EXOS_AGENT_CLIENT=desktop",
    `export EXOS_AGENT_SERVER_USERNAME=${shellEscape(username)}`,
    `export EXOS_AGENT_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(exos-agent)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
