...
import { expectExosAgentVersion, pendingRestartAfterWslInstall, wslServerIdsToStartOnInitialize } from "./startup"
...
type WslServersControllerOptions = {
  ...
  resolveExosAgent?: typeof resolveWslExosAgent
...
export function createWslServersController(
  ...
  const setExosAgentCheck = (distro: string, check: WslExosAgentCheck) => {
    setState({
      exosAgentChecks: {
        ...state.exosAgentChecks,
        [distro]: check,
      },
    })
  }

  const checkExosAgent = async (distro: string, opts?: { signal?: AbortSignal }) => {
    const resolved = await (options?.resolveExosAgent ?? resolveWslExosAgent)(distro, opts)
    const version = resolved
      ? await (options?.readCommandVersion ?? readWslCommandVersion)(resolved, distro, opts)
      : null
    return exosAgentCheck(distro, resolved, version, appVersion)
  }

  const refreshExosAgentCheck = async (distro: string, opts?: { signal?: AbortSignal }) => {
    setExosAgentCheck(distro, await checkExosAgent(distro, opts))
  }
  ...
  const probeAddableDistros = async (distros: string[], opts?: { signal?: AbortSignal }) => {
    ...
    const exosAgentChecks = await Promise.all(
      unique
        .filter((distro) => distroProbeReady(state.distroProbes[distro]))
        .filter((distro) => !state.exosAgentChecks[distro])
        .map(async (distro) => [distro, await checkExosAgent(distro, opts)] as const),
    )
    if (exosAgentChecks.length) {
      setState({ exosAgentChecks: { ...state.exosAgentChecks, ...Object.fromEntries(exosAgentChecks) } })
    }
  }
  ...
  const refreshExosAgentCheckBackground = (id: string, distro: string) => {
    void checkExosAgent(distro)
      .then((check) => {
        if (!hasServer(id, distro)) return
        setExosAgentCheck(distro, check)
      })
      .catch((error) => {
        const message = error instanceof Error ? error.message : String(error)
        logger?.error("wsl exos-agent check failed", { id, distro, message })
      })
  }

  const refreshExosAgentChecks = async () => {
    await Promise.all(
      state.servers.map((item) =>
        checkExosAgent(item.config.distro)
          .then((check) => {
            if (!hasServer(item.config.id, item.config.distro)) return
            setExosAgentCheck(item.config.distro, check)
          })
          .catch((error) => {
            const message = error instanceof Error ? error.message : String(error)
            logger?.error("wsl exos-agent check failed", {
              id: item.config.id,
  ...
  const startServer = async (id: string) => {
    ...
    try {
      ...
      refreshExosAgentCheckBackground(id, item.config.distro)
  ...
  return {
    ...
    async installExosAgent(name: string) {
      await runJob({ kind: "install-exos-agent", distro: name, startedAt: Date.now() }, async (abort) => {
        const result = await installWslExosAgent(appVersion, name, { signal: abort.signal })
        if (result.code !== 0) {
          throw new Error(summarize(result.stderr || result.stdout) || "ExosAgent installation failed")
        }
        await refreshExosAgentCheck(name, { signal: abort.signal })
        expectExosAgentVersion(state.exosAgentChecks[name]?.version ?? null, appVersion, name)
    ...
    async removeServer(id: string) {
      ...
      setState({
        ...
        ...(distro ? clearWslDistroState(state.distroProbes, state.exosAgentChecks, distro) : {}),
      })
    },
...
function exosAgentCheck(
  ...
  if (!resolvedPath) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "exos-agent is not installed in this distro",
    }
  }
  if (!version) {
    return {
