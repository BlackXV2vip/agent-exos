...
`packages/cli`, or `@exos-agent/core`.
...
- Do not replace private `packages/exos-agent` imports with `@exos-agent/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@exos-agent/tui`.
...
Status: Completed. The private `@exos-agent/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@exos-agent/tui`.
...
- The package has no dependency on `exos-agent`, `@exos-agent/cli`, or
  `@exos-agent/core`.
...
primitives now live in `@exos-agent/tui`. The legacy host consumes explicit
...
and build inputs through `@exos-agent/tui/runtime`. Movable app, component,
...
`@exos-agent/tui/config` now owns schemas, defaults, keybind resolution, the
...
canonical in `@exos-agent/tui`. Configured references resolve through the new
...
`@exos-agent/tui`. The legacy host injects a narrow plugin host that retains
...
public `@exos-agent/tui` root while retaining worker/server/config/plugin and
...
- Narrow `@exos-agent/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@exos-agent/core`, or either
  executable package.
...
rg '@exos-agent/core|packages/exos-agent|packages/cli' packages/tui
